...
-- Copiando estrutura para função prod_kls.desbloquear
DELIMITER //
CREATE DEFINER=`fsantini`@`%` FUNCTION `desbloquear`(
	`P_IDANH` INT,
...
BEGIN
	...
	IF (P_ROLE='student') THEN 
		UPDATE mdl_user_enrolments SET STATUS=0 WHERE userid=P_USERID AND enrolid=P_ENROLID;
	END IF;												

	RETURN 'Matricula Desbloqueada com sucesso';
