...
-- Copiando estrutura para procedure prod_kls.PRC_MATERIAL_COMP_AMP
DELIMITER //
CREATE DEFINER=`jsilva`@`%` PROCEDURE `PRC_MATERIAL_COMP_AMP`()
BEGIN
	...
	CREATE INDEX idx_coord_amp ON tbl_coord_amp (shortname, grupo);
			...
			INSERT INTO tbl_tmp_matcomplementar
			  ...
			  FROM (
						...
						where tc.visible=1 -- and tc.userid=32190 and tc.courseid=4125 
		...
		INSERT INTO tbl_tmp_material
			...
			INSERT INTO tbl_material_comp_amp
				SELECT 	rg.nm_regional AS REGIONAL,
							...
							material.Turma,
							material.Encontro,
						   material.QTD_MAT AS QTD_PRE, -- IF(material.aulacod = 2, 'Pré Aula', IF(material.aulacod = 4,'Aula', IF(material.aulacod = 6,'Pós Aula',''))) AS 'Pré/Aula/Pos',
							0 AS QTD_AULA,
							0 AS QTD_POS,
				...
				WHERE material.aulacod=2
				 ...
				 -- AND ins.cd_instituicao = 'OLIM-535'
			...
			INSERT INTO tbl_material_comp_amp
				SELECT 	rg.nm_regional AS REGIONAL,
							...
							material.Turma,
							material.Encontro,
							0 AS QTD_PRE, -- IF(material.aulacod = 2, 'Pré Aula', IF(material.aulacod = 4,'Aula', IF(material.aulacod = 6,'Pós Aula',''))) AS 'Pré/Aula/Pos',
 ...
 /*  	  UPDATE tbl_material_comp_amp mca
		  JOIN tbl_mdl_user u on mca.PROFESSOR=concat(u.firstname,' ',u.lastname)
		  JOIN tbl_tmp_material mat ON mat.CS_SHORTNAME = mca.SHORTNAME 
		                           AND mat.GR_DESCRIPTION = mca.TURMA
		                           AND mat.ENCONTRO = mca.ENCONTRO
		                           AND mat.PROFESSOR = u.id
			SET mca.QTD_AULA = mat.QTD_MAT,
			    mca.TEMPO_AULA = mat.TEMPO
		 WHERE mat.AULACOD = 4;
		...
		UPDATE tbl_material_comp_amp mca
		  JOIN tbl_tmp_material mat ON mat.CS_SHORTNAME = mca.SHORTNAME 
			                        AND mat.GR_DESCRIPTION = mca.TURMA
			                        AND mat.ENCONTRO = mca.ENCONTRO
			                        AND mat.PROFESSOR = mca.PROFESSOR
			SET mca.QTD_POS = mat.QTD_MAT,
			    mca.TEMPO_POS = mat.TEMPO
		 WHERE mat.AULACOD = 6;*/
