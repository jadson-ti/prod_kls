...
-- Copiando estrutura para procedure prod_kls.PRC_INDICADORES_AMI
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_INDICADORES_AMI`()
BEGIN
   ...
   IF SUMMERTIME=1 THEN 
	...
	SELECT database(), 'PRC_INDICADORES_AMI', user(), sysdate(), 'PROCESSANDO' FROM DUAL;
	SET ID_LOG_=LAST_INSERT_ID();
	
    DROP TABLE IF EXISTS carga_base_tutor_ami;
 
    CREATE TABLE carga_base_tutor_ami (
                                             COURSE                 BIGINT(10),
                            				 		GROUP_ID           	BIGINT(10), 
			  											   TUTOR_ID               BIGINT(10), 
	...
	ALTER TABLE carga_base_tutor_ami ADD PRIMARY KEY (COURSE, GROUP_ID);
	
    DROP TABLE IF EXISTS tbl_indicadores_ami;
 
    CREATE TABLE tbl_indicadores_ami ( COD_UNIDADE             	 VARCHAR(100) DEFAULT 'Nenhum Encontrado',
	...
	CREATE INDEX idx_CURGRP_AMI ON tbl_indicadores_ami (ID_CURSO, ID_GRUPO);
	CREATE INDEX idx_CUR_AMI ON tbl_indicadores_ami (ID_CURSO);
	CREATE INDEX idx_GRP_AMI ON tbl_indicadores_ami (ID_GRUPO);
	CREATE INDEX idx_GRP_LOGIN ON tbl_indicadores_ami (LOGIN);
	CREATE INDEX idx_GRP_UC ON tbl_indicadores_ami (USERID, ID_CURSO);

				
					INSERT INTO carga_base_tutor_ami
							        SELECT DISTINCT 
												c.id AS COURSE, 
												g.id AS GROUP_ID, 
							      ...
							      FROM anh_academico_matricula PARTITION (AMI) anh
							      ...
							      JOIN mdl_course c on c.shortname=anh.shortname
							        ...
							        WHERE 
									   anh.shortname  REGEXP '^AMI' AND anh.role='editingteacher'  
									GROUP BY c.id, g.id;


		  		INSERT INTO tbl_indicadores_ami 
				(
            	...
            	FROM mdl_context ct 
					...
					WHERE ct.contextlevel = 50 AND c.shortname REGEXP '^AMI';

			UPDATE tbl_indicadores_ami t 
	     ...
	     SET t.PrimeiroAcessoDisciplina = IFNULL((SELECT IFNULL(FROM_UNIXTIME(MIN(l.DT)-FUSOHORARIO,'%d/%m/%Y %H:%i:%s'),'Nunca Acessou')
     		...
     		SET t.UltimoAcessoDisciplina = IFNULL((SELECT IFNULL(FROM_UNIXTIME(MAX(l.DT)-FUSOHORARIO,'%d/%m/%Y %H:%i:%s'),'Nunca Acessou')
	  		...
	  		SET t.QuantDiasAcesso = IFNULL((SELECT COUNT(DISTINCT DATE_FORMAT(FROM_UNIXTIME(l.DT),'%d/%m/%Y')) 
				...
				UPDATE tbl_indicadores_ami t
        		JOIN carga_base_tutor_ami bt ON bt.GROUP_ID = t.ID_GRUPO AND bt.COURSE = t.ID_CURSO
		      SET t.TUTOR_ID = bt.TUTOR_ID, 
	         	...
	         	UPDATE tbl_indicadores_ami t
					...
					drop table if exists tmp_avaliacao_ami;
					create temporary table tmp_avaliacao_ami (
						userid bigint(10),
						course bigint(10),
						assign bigint(10),
						lastsub bigint(20),
					...
					alter table tmp_avaliacao_ami add index idx_uc (userid, course);


					insert into tmp_avaliacao_ami
							select 
						...
						join tbl_indicadores_ami tcc on tcc.USERID=asu.userid and tcc.ID_CURSO=ma.course 
					...
					update tbl_indicadores_ami t 
					join tmp_avaliacao_ami b on t.USERID=b.userid and t.ID_CURSO=b.course
			     ...
			     UPDATE tbl_indicadores_ami A
				  SET ATIVIDADE_1_NOTA=-2,
				  		ATIVIDADE_1_AVALIACAO=-2,
						ATIVIDADE_1_NOTA=-2 
				  WHERE NOME_CURSO IN (
				    SELECT DISTINCT c.shortname
				    FROM mdl_course c 
				    LEFT JOIN mdl_assign a ON a.course=c.id AND a.name = 'Atividade Discursiva'
				    WHERE a.id IS NULL AND c.shortname REGEXP '^AMI_'
				  );
              ...
              DEALLOCATE PREPARE stmt;                          
			     ...
			     UPDATE tbl_indicadores_ami A
				  ...
				  WHERE NOME_CURSO IN (
				    ...
				    WHERE q.id IS NULL AND c.shortname REGEXP ''^AMI_''
				  );
				  '
				  );
							PREPARE stmt FROM @query;
      ...
      WHILE U_QUIZ_ < 4 DO
          ...
          SET S_QUIZ_ = 0;
            ...
            WHILE S_QUIZ_ < 4 DO
                ...
                SET @query = CONCAT('UPDATE tbl_indicadores_ami t 
	...
	update  tbl_indicadores_ami set atividade_1_aluno='Nunca Acessou' WHERE atividade_1_aluno IS NULL;
...
where a.UltimoAcessoDisciplina='Nunca Acessou' and
	...
	UPDATE tbl_prc_log pl
		SET pl.FIM = sysdate(),
	 		 pl.`STATUS` = CODE_, 
	 		 pl.REGISTROS = IFNULL(REGISTROS_, (SELECT COUNT(1) FROM tbl_indicadores_ami)),
	 		 pl.INFO = MSG_
	 WHERE pl.ID = ID_LOG_;
