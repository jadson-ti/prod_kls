...
-- Copiando estrutura para procedure prod_kls.PRC_INDICADORES_ED1_DDC
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_INDICADORES_ED1_DDC`()
MAIN: BEGIN
   ...
   DECLARE CONTINUE HANDLER FOR SQLEXCEPTION
	...
	INSERT INTO tbl_prc_log (BANCO, OBJETO, AUTOR, INICIO, `STATUS`)
		SELECT database(), 'PRC_INDICADORES_ED1_DDC', user(), sysdate(), 'PROCESSANDO' FROM DUAL;
   ...
   SELECT id INTO COURSE_ FROM mdl_course WHERE SHORTNAME='ED_DDC';
   
   DROP TABLE IF EXISTS tbl_indicadores_ed1_ddc;

   CREATE TABLE tbl_indicadores_ed1_ddc (  COD_UNIDADE						VARCHAR(100) DEFAULT 'Nenhum Encontrado',
 	  ...
 	  CREATE INDEX idx_CURSO ON tbl_indicadores_ed1_ddc (ID_CURSO, ID_GRUPO);        
     CREATE INDEX idx_USER ON tbl_indicadores_ed1_ddc (ID_CURSO, USERID); 

		DROP TABLE IF EXISTS tmp_log_ddc_url;
		CREATE TABLE `tmp_log_ddc_url` (
		...
		PRIMARY KEY (`id`),
		INDEX `tmp_logsstanlog_tim_ix` (`timecreated`),
		INDEX `tmp_logsstanlog_coueve_ix` (`userid`, objectid)
	);

	INSERT INTO tmp_log_ddc_url
	SELECT * FROM mdl_logstore_standard_log WHERE courseid=COURSE_ AND objecttable='url';

   INSERT INTO tbl_indicadores_ed1_ddc (COD_UNIDADE, UNIDADE, ID_CURSO, NOME_CURSO, FULLNAME, ID_Categoria, 
					       ...
					       WHERE c.shortname='ED_DDC'  and g.name REGEXP 'TUTOR'
			...
			UPDATE tbl_indicadores_ed1_ddc t 
		  ...
		  WHILE N_ASSIGN_ < 9 DO
		  
		  		SET N_ASSIGN_ = N_ASSIGN_ + 1;
		  
			  	  
			  	    SET @item = CASE	
										WHEN N_ASSIGN_=1 THEN 'Carreira e Profissão - Talento e vocação'
										WHEN N_ASSIGN_=2 THEN 'Transformando a formação em carreira'
										WHEN N_ASSIGN_=3 THEN 'Minhas habilidades'
										WHEN N_ASSIGN_=4 THEN 'Ranking de competências'
										WHEN N_ASSIGN_=5 THEN 'Desenvolvendo competências'
										WHEN N_ASSIGN_=6 THEN 'Construindo um plano de carreira'
										WHEN N_ASSIGN_=7 THEN 'Passo a passo do plano de carreira'
										WHEN N_ASSIGN_=8 THEN 'Ferramentas do profissionais do futuro'
										WHEN N_ASSIGN_=9 THEN 'Guia Complementar'
							    	 END;
			         
					SET @moduleid = (select u.id from mdl_url u join mdl_modules m on m.name='url' 	
									join mdl_course c on c.id=u.course and c.shortname='ED_DDC'
									join mdl_course_modules cm on cm.module=m.id and cm.instance=u.id and cm.course=u.course
									where u.name=@item limit 1);

		            SET @campo = CONCAT('URL_', N_ASSIGN_, '_ACESSO');
		         	
					SET @query = CONCAT('UPDATE tbl_indicadores_ed1_ddc t
												SET t.', @campo, ' = (
																		SELECT FROM_UNIXTIME(MAX(timecreated)-',FUSOHORARIO,',''%d/%m/%Y %H:%i:%s'')
																		FROM tmp_log_ddc_url 
																		WHERE userid=t.USERID and objectid=',@moduleid,'
																	 )');
			...
			update tbl_indicadores_ed1_ddc set URL_1_ACESSO='Pendente' WHERE URL_1_ACESSO IS NULL;
			update tbl_indicadores_ed1_ddc set URL_2_ACESSO='Pendente' WHERE URL_2_ACESSO IS NULL;
			update tbl_indicadores_ed1_ddc set URL_3_ACESSO='Pendente' WHERE URL_3_ACESSO IS NULL;
			update tbl_indicadores_ed1_ddc set URL_4_ACESSO='Pendente' WHERE URL_4_ACESSO IS NULL;
			update tbl_indicadores_ed1_ddc set URL_5_ACESSO='Pendente' WHERE URL_5_ACESSO IS NULL;
			update tbl_indicadores_ed1_ddc set URL_6_ACESSO='Pendente' WHERE URL_6_ACESSO IS NULL;
			update tbl_indicadores_ed1_ddc set URL_7_ACESSO='Pendente' WHERE URL_7_ACESSO IS NULL;
			update tbl_indicadores_ed1_ddc set URL_8_ACESSO='Pendente' WHERE URL_8_ACESSO IS NULL;
			update tbl_indicadores_ed1_ddc set URL_9_ACESSO='Pendente' WHERE URL_9_ACESSO IS NULL;
	...
	UPDATE tbl_prc_log pl
		SET pl.FIM = sysdate(),
	 		 pl.`STATUS` = CODE_, 
	 		 pl.REGISTROS = IFNULL(REGISTROS_, (SELECT COUNT(1) FROM tbl_indicadores_ed1_ddc)),
	 		 pl.INFO = MSG_
	 WHERE pl.ID = ID_LOG_;
