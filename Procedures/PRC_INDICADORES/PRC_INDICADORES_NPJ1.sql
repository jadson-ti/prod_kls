...
-- Copiando estrutura para procedure prod_kls.PRC_INDICADORES_NPJ1
DELIMITER //
CREATE DEFINER=`jsilva`@`%` PROCEDURE `PRC_INDICADORES_NPJ1`()
main: BEGIN
	...
	INSERT INTO tbl_prc_log (BANCO, OBJETO, AUTOR, INICIO, `STATUS`)
		SELECT database(), 'PRC_INDICADORES_NPJ1', user(), sysdate(), 'PROCESSANDO' FROM DUAL;
   ...
   IF SUMMERTIME=1 THEN 
	...
	END IF;		  
	  ...
	  DROP TABLE IF EXISTS carga_base_tutor_npj1;
	
	  CREATE TEMPORARY TABLE carga_base_tutor_npj1 ( TUTOR_ID               BIGINT, 
	  ...
	  CREATE INDEX idx_GROUP_ID ON carga_base_tutor_npj1 (GROUP_ID);
	
	  INSERT INTO carga_base_tutor_npj1
	  ...
	  AND c.fullname = 'Estágio Supervisionado I' and c.shortname LIKE 'NPJ%';
	
	  DROP TABLE IF EXISTS tbl_indicadores_npj1;
	
	  CREATE TABLE tbl_indicadores_npj1 ( COD_UNIDADE									VARCHAR(100),
	  												 UNIDADE											VARCHAR(100),
	  												 ID_CURSO_KLS									BIGINT,
	  ...
	  CREATE INDEX idx_USERID ON tbl_indicadores_npj1 (USERID);
		   
        INSERT INTO tbl_indicadores_npj1
                SELECT DISTINCT i.cd_instituicao AS COD_UNIDADE,
					 			i.nm_ies AS UNIDADE,
					 			kc.id_curso AS ID_CURSO_KLS,
								kc.cd_curso AS COD_CURSO_KLS,
								kc.nm_curso AS NOME_CURSO_KLS,
					 			c.id AS ID_CURSO, 
        ...
        WHERE c.fullname = 'Estágio Supervisionado I' AND c.shortname LIKE 'NPJ%'
		  AND g.name REGEXP 'TUTOR';
     
	  		UPDATE tbl_indicadores_npj1 t
	  ...
	  UPDATE tbl_indicadores_npj1 t
	  SET t.QuantDiasAcesso = IFNULL((SELECT COUNT(DISTINCT DATE_FORMAT(FROM_UNIXTIME(l.timecreated),'%d/%m/%Y')) 
		 											FROM mdl_logstore_standard_log l
												  WHERE l.USERID = t.USERID 
		  ...
		  WHILE N_ASSIGN_ < 6 DO
	         	...
	         	SET @campo = CONCAT('ATIVIDADE_', N_ASSIGN_, '_ALUNO_PENAL'); 
	           	...
	           	SET @query = CONCAT('UPDATE tbl_indicadores_npj1 t
	     ...
	     END WHILE;
		  ...
		  WHILE N_ASSIGN_ < 6 DO
	         	...
	         	SET @campo = CONCAT('ATIVIDADE_', N_ASSIGN_, '_AVALIACAO_PENAL'); 
					drop table if exists tmp_avaliacao_npj1;
					create temporary table tmp_avaliacao_npj1 (
						userid bigint(10),
						course bigint(10),
						assign bigint(10),
						lastsub bigint(20),
					...
					insert into tmp_avaliacao_npj1
							select 
						...
						join tbl_indicadores_npj1 tcc on tcc.USERID=asu.userid and tcc.ID_CURSO=ma.course 
					...
					set @query=concat('update tbl_indicadores_npj1 t join tmp_avaliacao_npj1 b on t.USERID=b.userid and t.ID_CURSO=b.course
	     ...
	     END WHILE; 
				...
				UPDATE tbl_indicadores_npj1 t
        		JOIN carga_base_tutor_npj1 bt ON bt.GROUP_ID = t.ID_GRUPO AND bt.COURSE = t.ID_CURSO
	  ...
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_1_ALUNO_PENAL = 'Pendente' WHERE ATIVIDADE_1_ALUNO_PENAL is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_1_ALUNO_TRABALHISTA = 'Pendente' WHERE ATIVIDADE_1_ALUNO_TRABALHISTA is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_2_ALUNO_PENAL = 'Pendente' WHERE ATIVIDADE_2_ALUNO_PENAL is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_2_ALUNO_TRABALHISTA = 'Pendente' WHERE ATIVIDADE_2_ALUNO_TRABALHISTA is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_3_ALUNO_PENAL = 'Pendente' WHERE ATIVIDADE_3_ALUNO_PENAL is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_3_ALUNO_TRABALHISTA = 'Pendente' WHERE ATIVIDADE_3_ALUNO_TRABALHISTA is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_4_ALUNO_PENAL = 'Pendente' WHERE ATIVIDADE_4_ALUNO_PENAL is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_4_ALUNO_TRABALHISTA = 'Pendente' WHERE ATIVIDADE_4_ALUNO_TRABALHISTA is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_5_ALUNO_PENAL = 'Pendente' WHERE ATIVIDADE_5_ALUNO_PENAL is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_5_ALUNO_TRABALHISTA = 'Pendente' WHERE ATIVIDADE_5_ALUNO_TRABALHISTA is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_6_ALUNO_PENAL = 'Pendente' WHERE ATIVIDADE_6_ALUNO_PENAL is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_6_ALUNO_TRABALHISTA = 'Pendente' WHERE ATIVIDADE_6_ALUNO_TRABALHISTA is null;

	  
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_1_AVALIACAO_PENAL = 'Pendente' WHERE ATIVIDADE_1_AVALIACAO_PENAL is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_2_AVALIACAO_PENAL = 'Pendente' WHERE ATIVIDADE_2_AVALIACAO_PENAL is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_3_AVALIACAO_PENAL = 'Pendente' WHERE ATIVIDADE_3_AVALIACAO_PENAL is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_4_AVALIACAO_PENAL = 'Pendente' WHERE ATIVIDADE_4_AVALIACAO_PENAL is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_5_AVALIACAO_PENAL = 'Pendente' WHERE ATIVIDADE_5_AVALIACAO_PENAL is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_6_AVALIACAO_PENAL = 'Pendente' WHERE ATIVIDADE_5_AVALIACAO_PENAL is null;

	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_1_AVALIACAO_TRABALHISTA = 'Pendente' WHERE ATIVIDADE_1_AVALIACAO_TRABALHISTA is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_2_AVALIACAO_TRABALHISTA = 'Pendente' WHERE ATIVIDADE_2_AVALIACAO_TRABALHISTA is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_3_AVALIACAO_TRABALHISTA = 'Pendente' WHERE ATIVIDADE_3_AVALIACAO_TRABALHISTA is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_4_AVALIACAO_TRABALHISTA = 'Pendente' WHERE ATIVIDADE_4_AVALIACAO_TRABALHISTA is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_5_AVALIACAO_TRABALHISTA = 'Pendente' WHERE ATIVIDADE_5_AVALIACAO_TRABALHISTA is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_6_AVALIACAO_TRABALHISTA = 'Pendente' WHERE ATIVIDADE_5_AVALIACAO_TRABALHISTA is null;

	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_1_NOTA_PENAL = 'Pendente' WHERE ATIVIDADE_1_NOTA_PENAL is null;
  	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_2_NOTA_PENAL = 'Pendente' WHERE ATIVIDADE_2_NOTA_PENAL is null;
 	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_3_NOTA_PENAL = 'Pendente' WHERE ATIVIDADE_3_NOTA_PENAL is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_4_NOTA_PENAL = 'Pendente' WHERE ATIVIDADE_4_NOTA_PENAL is null;
 	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_5_NOTA_PENAL = 'Pendente' WHERE ATIVIDADE_5_NOTA_PENAL is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_6_NOTA_PENAL = 'Pendente' WHERE ATIVIDADE_5_NOTA_PENAL is null;

	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_1_NOTA_TRABALHISTA = 'Pendente' WHERE ATIVIDADE_1_NOTA_TRABALHISTA is null;
  	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_2_NOTA_TRABALHISTA = 'Pendente' WHERE ATIVIDADE_2_NOTA_TRABALHISTA is null;
 	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_3_NOTA_TRABALHISTA = 'Pendente' WHERE ATIVIDADE_3_NOTA_TRABALHISTA is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_4_NOTA_TRABALHISTA = 'Pendente' WHERE ATIVIDADE_4_NOTA_TRABALHISTA is null;
 	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_5_NOTA_TRABALHISTA = 'Pendente' WHERE ATIVIDADE_5_NOTA_TRABALHISTA is null;
	  UPDATE tbl_indicadores_npj1 SET ATIVIDADE_6_NOTA_TRABALHISTA = 'Pendente' WHERE ATIVIDADE_5_NOTA_TRABALHISTA is null;

update tbl_indicadores_npj1 a 
...
update tbl_indicadores_npj1 a 
set PrimeiroAcessoDisciplina='Nunca Acessou' 
where PrimeiroAcessoDisciplina is null;

update tbl_indicadores_npj1 a 
set UltimoAcessoDisciplina='Nunca Acessou' 
where UltimoAcessoDisciplina is null;
		 
	 UPDATE tbl_prc_log pl
	 SET pl.FIM = sysdate(),
	 		 pl.`STATUS` = CODE_, 
	 		 pl.REGISTROS = IFNULL(REGISTROS_, (SELECT COUNT(1) FROM tbl_indicadores_npj1)),
	 		 pl.INFO = MSG_
	 WHERE pl.ID = ID_LOG_;
