...
-- Copiando estrutura para procedure prod_kls.PRC_INDICADORES_EST
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_INDICADORES_EST`()
BEGIN
   ...
   DECLARE CONTINUE HANDLER FOR SQLEXCEPTION
	...
	INSERT INTO tbl_prc_log (BANCO, OBJETO, AUTOR, INICIO, `STATUS`)
		SELECT database(), 'PRC_INDICADORES_EST', user(), sysdate(), 'PROCESSANDO' FROM DUAL;
   ...
   IF SUMMERTIME=1 THEN 
	...
	DROP TABLE IF EXISTS carga_base_tutor_est;
	
	
	CREATE TEMPORARY TABLE carga_base_tutor_est ( TUTOR_ID               BIGINT, 
	...
	CREATE INDEX idx_GROUP_ID ON carga_base_tutor_est (COURSE, GROUP_ID);


	DROP TABLE IF EXISTS tbl_indicadores_est;

	CREATE TABLE tbl_indicadores_est ( COD_UNIDADE							VARCHAR(100) DEFAULT 'Nenhum Encontrado',
	...
	CREATE INDEX idx_USERID ON tbl_indicadores_est (ID_CURSO, ID_GRUPO);
		

				INSERT INTO carga_base_tutor_est
					SELECT DISTINCT u.id AS TUTOR_ID, u.username AS TUTOR_LOGIN, u.email AS TUTOR_EMAIL, 
							          ...
							          IFNULL(
										 			(
													 	SELECT IFNULL(FROM_UNIXTIME(MIN(l.DT)-FUSOHORARIO,'%d/%m/%Y %H:%i:%s'),'Nunca Acessou')
										  	  	   	FROM tbl_login l 
														WHERE l.userid = u.id AND l.course = c.id
										  			), 'Nunca Acessou'
										 ) AS TUTOR_PrimeiroAcesso,
							          (	
										 		SELECT COALESCE(FROM_UNIXTIME(MAX(l.DT)-FUSOHORARIO,'%d/%m/%Y %H:%i:%s'),'Nunca Acessou')
												FROM tbl_login l WHERE l.userid = u.id AND l.course = c.id
										 ) AS TUTOR_UltimoAcesso
					  				  FROM mdl_context ct 
							        ...
							        WHERE ct.contextlevel = 50 and c.shortname LIKE 'EST%';


	     
			  		INSERT INTO tbl_indicadores_est ( ID_CURSO, NOME_CURSO, FULLNAME, ID_Categoria, 
								                ...
								                SELECT DISTINCT 
								                        ...
								                        IFNULL(FROM_UNIXTIME(ra.timemodified-FUSOHORARIO,'%d/%m/%Y %H:%i:%s'),'Nunca Acessou') AS DataVinculoDisciplina
								        FROM mdl_course c 
								        JOIN mdl_context ct ON ct.instanceid = c.id
								        JOIN mdl_course_categories cc ON cc.id = c.category
								        JOIN mdl_role_assignments ra ON ra.contextid = ct.id AND ra.roleid = 5
								        JOIN mdl_user u ON u.id = ra.userid
								        JOIN mdl_groups g ON g.courseid = c.id
								        JOIN mdl_groups_members gm ON gm.groupid = g.id AND gm.userid = u.id
								        WHERE ct.contextlevel = 50 AND g.name REGEXP 'TUTOR' and c.shortname LIKE 'EST%'; 
				
     
     
			UPDATE tbl_indicadores_est t 
	  ...
	  SET t.PrimeiroAcessoDisciplina = IFNULL((SELECT IFNULL(FROM_UNIXTIME(MIN(l.DT)-FUSOHORARIO,'%d/%m/%Y %H:%i:%s'),'Nunca Acessou')
			  ...
			  UPDATE tbl_indicadores_est t
			     SET t.ATIVIDADE_1_ALUNO = (SELECT CASE
																	...
																	WHEN asub.status = 'reopened' then
																		CONCAT('Ultimo Envio ',(SELECT FROM_UNIXTIME(MAX(asub.timemodified)-FUSOHORARIO,'%d/%m/%Y')
																											...
																											WHERE ass.course = t.ID_CURSO
																												 	AND ass.name REGEXP 'Acompanhamento e'
										            ...
										            JOIN mdl_assign ass ON asub.assignment = ass.id
										            AND ass.name REGEXP 'Acompanhamento e'
										            WHERE asub.userid = t.USERID
										            AND ass.course = t.ID_CURSO
			  ...
			  UPDATE tbl_indicadores_est t
			     SET t.ATIVIDADE_3_ALUNO = (SELECT CASE
																	WHEN asub.status = 'submitted' then 
																	...
																	WHEN asub.status = 'reopened' then
																		CONCAT('Ultimo Envio ',(SELECT FROM_UNIXTIME(MAX(asub.timemodified)-FUSOHORARIO,'%d/%m/%Y')
																											...
																											JOIN mdl_assign_submission asub ON asub.assignment = ass.id
																											WHERE ass.course = t.ID_CURSO
																												 	AND ass.name  REGEXP 'Compromisso'
																													AND asub.status = 'submitted'
																													AND asub.userid = t.USERID))
																	ELSE 
																		'Pendente'
			  ...
			  UPDATE tbl_indicadores_est t
			     SET t.ATIVIDADE_5_ALUNO = (SELECT CASE
																	WHEN asub.status = 'submitted' then 
																		CONCAT('Postada em ',FROM_UNIXTIME(MAX(asub.timemodified)-FUSOHORARIO,'%d/%m/%Y'))
																	WHEN asub.status = 'reopened' then
																		CONCAT('Ultimo Envio ',(SELECT FROM_UNIXTIME(MAX(asub.timemodified)-FUSOHORARIO,'%d/%m/%Y')
					...
					drop table if exists tmp_avaliacao_est;
					create temporary table tmp_avaliacao_est (
						userid bigint(10),
						course bigint(10),
						assign bigint(10),
						lastsub bigint(20),
					...
					truncate table tmp_avaliacao_est;
					insert into tmp_avaliacao_est
							select 
						...
						join tbl_indicadores_est est on est.USERID=asu.userid and est.ID_CURSO=ma.course 
						...
						WHERE  ma.name REGEXP 'Acompanhamento e' AND asu.status <> 'new'
						GROUP BY asu.userid,asu.assignment;

						update tmp_avaliacao_est a
					...
					update tbl_indicadores_est t join tmp_avaliacao_est b on t.USERID=b.userid and t.ID_CURSO=b.course
					set ATIVIDADE_1_AVALIACAO_PENAL=b.situacao;	
								       	     
					UPDATE tbl_indicadores_est t 
					SET t.ATIVIDADE_1_NOTA_PENAL = (
								select coalesce((
									select 
											case
									...
									AND ma.name REGEXP 'Acompanhamento e'
					...
					update tbl_indicadores_est set ATIVIDADE_1_NOTA_PENAL='Nota: 0.0' where ATIVIDADE_1_NOTA_PENAL='Nota: 1.00';
					update tbl_indicadores_est set ATIVIDADE_1_NOTA_PENAL='Nota: 20.0' where ATIVIDADE_1_NOTA_PENAL='Nota: 2.00';
					...
					insert into tmp_avaliacao_est
						...
						join tbl_indicadores_est est on est.USERID=asu.userid and est.ID_CURSO=ma.course 
						left join mdl_assign_grades agt ON agt.assignment = ma.id and asu.userid = agt.userid and asu.attemptnumber=agt.attemptnumber and (agt.grade>=0 or agt.grade is null)
						WHERE  ma.name REGEXP 'Compromisso' AND asu.status <> 'new'
						GROUP BY asu.userid,asu.assignment;
					...
					update tbl_indicadores_est t join tmp_avaliacao_est b on t.USERID=b.userid and t.ID_CURSO=b.course
					set ATIVIDADE_3_AVALIACAO_PENAL=b.situacao;	
								       	     
					UPDATE tbl_indicadores_est t 
					SET t.ATIVIDADE_3_NOTA_PENAL = (
								select coalesce((
									select 
											case
												when agt.grade is not null and agt.grade >= 0 then 
									...
									join mdl_assign_grades agt ON agt.assignment = ma.id
																					...
																					AND agt.grade>=0
									WHERE ma.course = t.ID_CURSO
									AND ma.name REGEXP 'Compromisso'
									AND agt.userid = t.USERID
									order by agt.attemptnumber desc
									limit 1
								), 'Pendente')
					...
					update tbl_indicadores_est set ATIVIDADE_3_NOTA_PENAL='Nota: 0.0' where ATIVIDADE_3_NOTA_PENAL='Nota: 1.00';
					update tbl_indicadores_est set ATIVIDADE_3_NOTA_PENAL='Nota: 20.0' where ATIVIDADE_3_NOTA_PENAL='Nota: 2.00';
					...
					insert into tmp_avaliacao_est
						...
						join tbl_indicadores_est est on est.USERID=asu.userid and est.ID_CURSO=ma.course 
						left join mdl_assign_grades agt ON agt.assignment = ma.id and asu.userid = agt.userid and asu.attemptnumber=agt.attemptnumber and (agt.grade>=0 or agt.grade is null)
						WHERE  ma.name REGEXP 'Final$' AND asu.status <> 'new'
						GROUP BY asu.userid,asu.assignment;
					...
					update tbl_indicadores_est t join tmp_avaliacao_est b on t.USERID=b.userid and t.ID_CURSO=b.course
					set ATIVIDADE_5_AVALIACAO_PENAL=b.situacao;	
								       	     
					UPDATE tbl_indicadores_est t 
					SET t.ATIVIDADE_5_NOTA_PENAL = (
								select coalesce((
									select 
											case
												when agt.grade is not null and agt.grade >= 0 then 
				...
				UPDATE tbl_indicadores_est t
        		  JOIN carga_base_tutor_est bt ON bt.GROUP_ID = t.ID_GRUPO AND bt.COURSE = t.ID_CURSO
	  ...
	  UPDATE tbl_indicadores_est SET ATIVIDADE_1_ALUNO = 'Pendente' WHERE ATIVIDADE_1_ALUNO is null;
	  
	  UPDATE tbl_indicadores_est SET ATIVIDADE_3_ALUNO = 'Pendente' WHERE ATIVIDADE_3_ALUNO is null;
	  
	  UPDATE tbl_indicadores_est SET ATIVIDADE_5_ALUNO = 'Pendente' WHERE ATIVIDADE_5_ALUNO is null;
	  
	  UPDATE tbl_indicadores_est SET ATIVIDADE_1_AVALIACAO_PENAL = 'Pendente' WHERE ATIVIDADE_1_AVALIACAO_PENAL is null;
	  
	  UPDATE tbl_indicadores_est SET ATIVIDADE_3_AVALIACAO_PENAL = 'Pendente' WHERE ATIVIDADE_3_AVALIACAO_PENAL is null;
	  
	  UPDATE tbl_indicadores_est SET ATIVIDADE_5_AVALIACAO_PENAL = 'Pendente' WHERE ATIVIDADE_5_AVALIACAO_PENAL is null;

	  UPDATE tbl_indicadores_est SET ATIVIDADE_1_NOTA_PENAL = 'Pendente' WHERE ATIVIDADE_1_NOTA_PENAL is null;
  	  
 	  UPDATE tbl_indicadores_est SET ATIVIDADE_3_NOTA_PENAL = 'Pendente' WHERE ATIVIDADE_3_NOTA_PENAL is null;
	  
 	  UPDATE tbl_indicadores_est SET ATIVIDADE_5_NOTA_PENAL = 'Pendente' WHERE ATIVIDADE_5_NOTA_PENAL is null;
	...
	UPDATE tbl_prc_log pl
		SET pl.FIM = sysdate(),
	 		 pl.`STATUS` = CODE_, 
	 		 pl.REGISTROS = IFNULL(REGISTROS_, (SELECT COUNT(1) FROM tbl_indicadores_est)),
	 		 pl.INFO = MSG_
	 WHERE pl.ID = ID_LOG_;
