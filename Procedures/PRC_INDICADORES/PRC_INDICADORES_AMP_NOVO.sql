...
-- Copiando estrutura para procedure prod_kls.PRC_INDICADORES_AMP_NOVO
DELIMITER //
CREATE DEFINER=`jsilva`@`%` PROCEDURE `PRC_INDICADORES_AMP_NOVO`()
BEGIN
   ...
   IF SUMMERTIME=1 THEN 
	...
	INSERT INTO carga_base_professor_coord (PROFESSOR_ID ,
							        ...
							        JOIN mdl_role r on r.id=ra.roleid and r.shortname  in ('editingteacher','coordteacher')
   ...
   DROP TABLE IF EXISTS tbl_indicadores_amp_novo;
 
   CREATE TABLE tbl_indicadores_amp_novo ( 
                              ...
                              COORDENADOR_ID                BIGINT DEFAULT 0, 
										COORDENADOR_LOGIN             VARCHAR(100) DEFAULT 'Sem Professor', 
                              ...
                              PROFESSOR_QuantAcesso         VARCHAR(100) DEFAULT 'Nunca Acessou',
										INDEX `IDX_CUR_IND_AMI` (`ID_CURSO`, `ID_GRUPO`),
										INDEX `IDX_USER` (`ID_CURSO`, `USERID`)
                              );   
    
	CREATE INDEX IDX_CUR_IND_AMI ON tbl_indicadores_amp_novo (ID_CURSO, ID_GRUPO);
	
	SET encontro:=1;
	
	WHILE (encontro<=16) DO
		SET @campo1:=CONCAT('ALTER TABLE tbl_indicadores_amp_novo ADD COLUMN Enc',encontro,'UltAcessoPre VARCHAR(100) DEFAULT ''Nunca Acessou''');
		SET @campo2:=CONCAT('ALTER TABLE tbl_indicadores_amp_novo ADD COLUMN Enc',encontro,'UltAcessoAula VARCHAR(100) DEFAULT ''Nunca Acessou''');
		SET @campo3:=CONCAT('ALTER TABLE tbl_indicadores_amp_novo ADD COLUMN Enc',encontro,'UltAcessoPos VARCHAR(100) DEFAULT ''Nunca Acessou''');
	...
	END WHILE; 
           ...
           -- AMP 
	  		INSERT INTO tbl_indicadores_amp_novo (COD_UNIDADE, 
																		UNIDADE, 
																		ID_CURSO, 
																		NOME_CURSO, 
																		FULLNAME, 
...
SELECT DISTINCT 
					        ...
					        JOIN kls_lms_tipo_modelo tp ON tp.id_tp_modelo=d.id_tp_modelo
							  JOIN kls_lms_curso kc ON kc.id_curso=cd.id_curso
                       ...
                       LEFT JOIN anh_disciplina_ami_amp temp on (temp.id_disciplina = d.id_disciplina)
		  				...
		  				SELECT DISTINCT 
					        ...
					        JOIN mdl_groups_members gm ON gm.groupid = g.id AND gm.userid = u.id
					        JOIN anh_aluno_matricula PARTITION (HAMPP) anh1 ON anh1.USERNAME=u.username and anh1.shortname=c.shortname
					        JOIN kls_lms_instituicao i2 ON i2.cd_instituicao=anh1.unidade
					        JOIN kls_lms_curso_disciplina cd on (cd.shortname = c.shortname AND cd.fl_etl_atu REGEXP '[NA]')
			  ...
			  UPDATE tbl_indicadores_amp_novo t 
			  JOIN anh_aluno_matricula anh ON anh.sigla='AMP' AND
			  											 anh.USERNAME=t.LOGIN AND
			  											 anh.SHORTNAME=t.NOME_CURSO
			  JOIN kls_lms_pessoa_curso_disc pcd ON pcd.id_pes_cur_disc=anh.ID_PES_CUR_DISC
	        JOIN kls_lms_curso_disciplina cd on cd.id_curso_disciplina=pcd.id_curso_disciplina
	        JOIN kls_lms_disciplina d ON d.id_disciplina=cd.id_disciplina
	        JOIN kls_lms_tipo_modelo tp ON tp.id_tp_modelo=d.id_tp_modelo
			  JOIN kls_lms_curso kc ON kc.id_curso=cd.id_curso
			  JOIN kls_lms_instituicao i ON i.id_ies=kc.id_ies
			   SET t.ID_CURSO_KLS = kc.id_curso,
				    t.COD_CURSO_KLS = kc.cd_curso,
				    t.NOME_CURSO_KLS = kc.nm_curso;
	  
      
     		UPDATE tbl_indicadores_amp_novo t
	  		SET 
			  	t.PrimeiroAcessoDisciplina = IFNULL((SELECT 
				  										 FROM_UNIXTIME(MIN(l.timecreated)-FUSOHORARIO,'%d/%m/%Y %H:%i:%s')
														FROM mdl_logstore_standard_log l 
														WHERE l.USERID=t.USERID AND l.courseid=t.ID_CURSO),'Nunca Acessou'),
  		   	t.UltimoAcessoDisciplina = IFNULL((SELECT 
				  										 FROM_UNIXTIME(MAX(l.timecreated)-FUSOHORARIO,'%d/%m/%Y %H:%i:%s')
														FROM mdl_logstore_standard_log l 
														WHERE l.USERID=t.USERID AND l.courseid=t.ID_CURSO),'Nunca Acessou');
    ...
    UPDATE tbl_indicadores_amp_novo
	...
	WHILE (encontro<=16) DO
		...
		SET @query1:=CONCAT('
		UPDATE tbl_indicadores_amp_novo amp
		JOIN tmp_curso_aula ac ON ac.course=amp.ID_CURSO AND ac.rotulo=0 AND ac.section=',encontro,'
		...
		SET @query2:=CONCAT('
		UPDATE tbl_indicadores_amp_novo amp
		JOIN tmp_curso_aula ac ON ac.course=amp.ID_CURSO AND ac.rotulo=1  AND ac.section=',encontro,'
		JOIN mdl_course_modules_completion l ON l.coursemoduleid=ac.coursemodules and amp.userid=l.userid and l.viewed=1
		...
		SET @query2:=CONCAT('
		UPDATE tbl_indicadores_amp_novo amp
		JOIN tmp_curso_aula ac ON ac.course=amp.ID_CURSO AND ac.rotulo=2  AND ac.section=',encontro,'
		JOIN mdl_course_modules_completion l ON l.coursemoduleid=ac.coursemodules and amp.userid=l.userid and l.viewed=1
	...
	UPDATE tbl_prc_log pl
		SET pl.FIM = sysdate(),
	 		 pl.`STATUS` = CODE_, 
	 		 pl.REGISTROS = IFNULL(REGISTROS_, (SELECT COUNT(1) FROM tbl_indicadores_amp_novo)),
	 		 pl.INFO = MSG_
	 WHERE pl.ID = ID_LOG_;
