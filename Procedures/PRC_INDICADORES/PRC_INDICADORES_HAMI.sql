...
-- Copiando estrutura para procedure prod_kls.PRC_INDICADORES_HAMI
DELIMITER //
CREATE DEFINER=`jsilva`@`%` PROCEDURE `PRC_INDICADORES_HAMI`()
BEGIN
   ...
   IF SUMMERTIME=1 THEN 
	...
	SELECT database(), 'PRC_INDICADORES_HAMI', user(), sysdate(), 'PROCESSANDO' FROM DUAL;
	SET ID_LOG_=LAST_INSERT_ID();
	
    DROP TABLE IF EXISTS carga_base_tutor_hami;
 
    CREATE TABLE carga_base_tutor_hami (
                                             COURSE                 BIGINT(10),
                            				 		GROUP_ID           	BIGINT(10), 
			  											   TUTOR_ID               BIGINT(10), 
	...
	ALTER TABLE carga_base_tutor_hami ADD PRIMARY KEY (COURSE, GROUP_ID);
	
	CREATE TABLE carga_base_coord_hami (
                                             COURSE                 BIGINT(10),
                            				 		GROUP_ID           	BIGINT(10), 
			  											   COORD_ID               BIGINT(10), 
  	...
  	ALTER TABLE carga_base_coord_hami ADD PRIMARY KEY (COURSE, GROUP_ID);
	
    DROP TABLE IF EXISTS tbl_indicadores_hami;
 
    CREATE TABLE tbl_indicadores_hami ( COD_UNIDADE             	 VARCHAR(100) DEFAULT 'Nenhum Encontrado',
                                      ...
                                      QUIZ_U1S4_DIAGNOSTICA        DECIMAL(3,1) DEFAULT 0,
                                      QUIZ_U1S4_APRENDIZAGEM       DECIMAL(3,1) DEFAULT 0,
                                      ...
                                      QUIZ_U2S4_DIAGNOSTICA        DECIMAL(3,1) DEFAULT 0,
                                      QUIZ_U2S4_APRENDIZAGEM       DECIMAL(3,1) DEFAULT 0,
                                      ...
                                      QUIZ_U3S4_DIAGNOSTICA        DECIMAL(3,1) DEFAULT 0,
                                      QUIZ_U3S4_APRENDIZAGEM       DECIMAL(3,1) DEFAULT 0,
                                      ...
                                      QUIZ_U4S4_DIAGNOSTICA        DECIMAL(3,1) DEFAULT 0,
                                      QUIZ_U4S4_APRENDIZAGEM       DECIMAL(3,1) DEFAULT 0,
	...
	CREATE INDEX idx_CURGRP_HAMI ON tbl_indicadores_hami (ID_CURSO, ID_GRUPO);
	CREATE INDEX idx_CUR_HAMI ON tbl_indicadores_hami (ID_CURSO);
	CREATE INDEX idx_GRP_HAMI ON tbl_indicadores_hami (ID_GRUPO);
	CREATE INDEX idx_GRP_LOGIN ON tbl_indicadores_hami (LOGIN);
	CREATE INDEX idx_GRP_UC ON tbl_indicadores_hami (USERID, ID_CURSO);

				
					INSERT INTO carga_base_tutor_hami
							        SELECT DISTINCT 
												c.id AS COURSE, 
												g.id AS GROUP_ID, 
							      ...
							      FROM anh_academico_matricula PARTITION (HAMIP) anh
							      ...
							      JOIN mdl_course c on c.shortname=anh.shortname
							        ...
							        WHERE 
									   anh.shortname  REGEXP '^HAMI' AND anh.role='editingteacher'  
									GROUP BY c.id, g.id;
									
									INSERT INTO carga_base_coord_hami
							        SELECT DISTINCT 
												c.id AS COURSE, 
												g.id AS GROUP_ID, 
												GROUP_CONCAT(DISTINCT u.id ORDER BY u.id ASC) AS COORD_ID, 
												GROUP_CONCAT(DISTINCT u.username ORDER BY u.id ASC) AS COORD_LOGIN, 
												GROUP_CONCAT(DISTINCT u.email ORDER BY u.id ASC) AS COORD_EMAIL, 
					               		DATE_FORMAT(FROM_UNIXTIME(MIN(gm.timeadded)),'%d/%m/%Y %H:%i:%s') AS COORD_DataVinculo,
					                     UPPER(GROUP_CONCAT(DISTINCT CONCAT(u.firstname, ' ',u.lastname) ORDER BY u.id ASC)) AS COORD_NOME,
												COALESCE(FROM_UNIXTIME(MIN(l.DT)-10800,'%d/%m/%Y %H:%i:%s'),'Nunca Acessou') AS COORD_PrimeiroAcesso,
												COALESCE(FROM_UNIXTIME(MAX(l.DT)-10800,'%d/%m/%Y %H:%i:%s'),'Nunca Acessou') AS COORD_UltimoAcesso
							      ...
							      JOIN mdl_course c on c.shortname=anh.shortname
									...
									LEFT JOIN tbl_login l on l.userid=u.id and l.course=c.id
							        WHERE 
									   anh.shortname REGEXP '^HAMI' AND anh.role='coordteacher'  
									GROUP BY c.id, g.id;


		  		INSERT INTO tbl_indicadores_hami 
				(
				 COD_UNIDADE, UNIDADE, ID_CURSO, NOME_CURSO, FULLNAME, ID_CATEGORIA,
				 ID_CURSO_KLS, COD_CURSO_KLS, NOME_CURSO_KLS,
 				 NOME_CATEGORIA, IDNUMBER_GRUPO, ID_GRUPO, GRUPO, USERID, LOGIN, NOME, 
            	...
            	FROM mdl_context ct 
					...
					WHERE ct.contextlevel = 50 AND c.shortname REGEXP '^HAMI';

			UPDATE tbl_indicadores_hami t 
	     ...
	     SET t.PrimeiroAcessoDisciplina = IFNULL((SELECT IFNULL(FROM_UNIXTIME(MIN(l.DT)-FUSOHORARIO,'%d/%m/%Y %H:%i:%s'),'Nunca Acessou')
     		...
     		SET t.UltimoAcessoDisciplina = IFNULL((SELECT IFNULL(FROM_UNIXTIME(MAX(l.DT)-FUSOHORARIO,'%d/%m/%Y %H:%i:%s'),'Nunca Acessou')
	  		...
	  		SET t.QuantDiasAcesso = IFNULL((SELECT COUNT(DISTINCT DATE_FORMAT(FROM_UNIXTIME(l.DT),'%d/%m/%Y')) 
				...
				UPDATE tbl_indicadores_hami t
        		JOIN carga_base_tutor_hami bt ON bt.GROUP_ID = t.ID_GRUPO AND bt.COURSE = t.ID_CURSO
		      SET t.TUTOR_ID = bt.TUTOR_ID, 
					 ...
					 UPDATE tbl_indicadores_hami t
        		JOIN carga_base_coord_hami bts ON bts.GROUP_ID = t.ID_GRUPO AND bts.COURSE = t.ID_CURSO
		      SET t.COORD_ID = bts.COORD_ID, 
					 t.COORD_LOGIN = bts.COORD_LOGIN, 
					 t.COORD_EMAIL = bts.COORD_EMAIL, 
					 t.COORD_DataVinculo = bts.COORD_DataVinculo, 
					 t.COORD_NOME = bts.COORD_NOME, 
					 t.COORD_PrimeiroAcesso = bts.COORD_PrimeiroAcesso, 
					 t.COORD_UltimoAcesso = bts.COORD_UltimoAcesso;
      ...
      WHILE U_QUIZ_ < 4 DO
          ...
          SET S_QUIZ_ = 0;
            ...
            WHILE S_QUIZ_ < 4 DO
                ...
                SET @query = CONCAT('UPDATE tbl_indicadores_hami t 
...
where a.UltimoAcessoDisciplina='Nunca Acessou' and
	...
	UPDATE tbl_prc_log pl
		SET pl.FIM = sysdate(),
	 		 pl.`STATUS` = CODE_, 
	 		 pl.REGISTROS = IFNULL(REGISTROS_, (SELECT COUNT(1) FROM tbl_indicadores_hami)),
	 		 pl.INFO = MSG_
	 WHERE pl.ID = ID_LOG_;
