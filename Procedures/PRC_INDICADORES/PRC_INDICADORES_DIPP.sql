...
-- Copiando estrutura para procedure prod_kls.PRC_INDICADORES_DIPP
DELIMITER //
CREATE DEFINER=`mveras`@`%` PROCEDURE `PRC_INDICADORES_DIPP`()
BEGIN
   ...
   IF SUMMERTIME=1 THEN 
	...
	SELECT database(), 'PRC_INDICADORES_DIPP', user(), sysdate(), 'PROCESSANDO' FROM DUAL;
	SET ID_LOG_=LAST_INSERT_ID();
	
    DROP TABLE IF EXISTS carga_base_prof_dipp;
 
    CREATE TABLE carga_base_prof_dipp (
                                             COURSE                 BIGINT(10),
                            				 		GROUP_ID           	BIGINT(10), 
			  											   TUTOR_ID               BIGINT(10), 
	...
	ALTER TABLE carga_base_prof_dipp ADD PRIMARY KEY (COURSE, GROUP_ID);

    CREATE TABLE carga_base_coord_dipp (
                                             COURSE                 BIGINT(10),
                            				 		GROUP_ID           	BIGINT(10), 
			  											   COORD_ID               BIGINT(10), 
	...
	ALTER TABLE carga_base_coord_dipp ADD PRIMARY KEY (COURSE, GROUP_ID);
	

    DROP TABLE IF EXISTS tbl_indicadores_dipp;
 
    CREATE TABLE tbl_indicadores_dipp ( 
                                      ...
                                      ATIVIDADE_1_NOTA             DECIMAL(3,1) DEFAULT 0,
                                      ...
                                      QUIZ_ROTEIRO_ESTUDO			 DECIMAL(3,1) DEFAULT 0,
	...
	CREATE INDEX idx_CURGRP_dipp ON tbl_indicadores_dipp (ID_CURSO, ID_GRUPO);
	CREATE INDEX idx_CUR_dipp ON tbl_indicadores_dipp (ID_CURSO);
	CREATE INDEX idx_GRP_dipp ON tbl_indicadores_dipp (ID_GRUPO);
	CREATE INDEX idx_GRP_LOGIN ON tbl_indicadores_dipp (LOGIN);
	CREATE INDEX idx_GRP_UC ON tbl_indicadores_dipp (USERID, ID_CURSO);

				
					INSERT INTO carga_base_prof_dipp
							        SELECT DISTINCT 
												c.id AS COURSE, 
												g.id AS GROUP_ID, 
							      ...
							      JOIN mdl_course c on c.shortname=anh.shortname
							        ...
							        WHERE 
									   anh.shortname REGEXP '^DIPP\_' AND anh.role='editingteacher'  
									   AND anh.sigla IN ('DI','DIP')
									GROUP BY c.id, g.id;

					INSERT INTO carga_base_coord_dipp
							        SELECT DISTINCT 
												c.id AS COURSE, 
												g.id AS GROUP_ID, 
												GROUP_CONCAT(DISTINCT u.id ORDER BY u.id ASC) AS TUTOR_ID, 
							      ...
							      JOIN mdl_course c on c.shortname=anh.shortname
									...
									LEFT JOIN tbl_login l on l.userid=u.id and l.course=c.id
							        WHERE 
									   anh.shortname REGEXP '^DIPP\_' AND anh.role='coordteacher'  
									   AND anh.sigla IN ('DI','DIP')
									GROUP BY c.id, g.id;

		  
    DROP TABLE IF EXISTS carga_base_tutor_dipp;

    CREATE TEMPORARY TABLE carga_base_tutor_dipp ( TUTOR_ID               BIGINT, 
                                                  TUTOR_LOGIN            VARCHAR(100), 
                                                  TUTOR_EMAIL            VARCHAR(100), 
                                                  COURSE                 BIGINT,
																  GROUP_ID					 BIGINT, 
    ...
    CREATE INDEX idx_GROUP_ID ON carga_base_tutor_dipp (COURSE, GROUP_ID);
	
    
    INSERT INTO carga_base_tutor_dipp
        ...
        WHERE ct.contextlevel = 50
		  AND c.shortname REGEXP '^DIPP\_';


		  		INSERT INTO tbl_indicadores_dipp 
				(
            	...
            	FROM mdl_context ct 
					...
					WHERE ct.contextlevel = 50 AND c.shortname REGEXP '^DIPP\_';

			UPDATE tbl_indicadores_dipp t 
		  ...
		  UPDATE tbl_indicadores_dipp bigdi
	     ...
	     SET t.PrimeiroAcessoDisciplina = IFNULL((SELECT IFNULL(FROM_UNIXTIME(MIN(l.DT)-FUSOHORARIO,'%d/%m/%Y %H:%i:%s'),'Nunca Acessou')
     		...
     		SET t.UltimoAcessoDisciplina = IFNULL((SELECT IFNULL(FROM_UNIXTIME(MAX(l.DT)-FUSOHORARIO,'%d/%m/%Y %H:%i:%s'),'Nunca Acessou')
	  		...
	  		SET t.QuantDiasAcesso = IFNULL((SELECT COUNT(DISTINCT DATE_FORMAT(FROM_UNIXTIME(l.DT),'%d/%m/%Y')) 
				...
				UPDATE tbl_indicadores_dipp t
        		JOIN carga_base_prof_dipp bt ON bt.GROUP_ID = t.ID_GRUPO_PRESENCIAL AND bt.COURSE = t.ID_CURSO
		      SET t.PROF_ID = bt.TUTOR_ID, 
				...
				UPDATE tbl_indicadores_dipp t
        		JOIN carga_base_tutor_dipp bt ON bt.GROUP_ID = t.ID_GRUPO_VIRTUAL AND bt.COURSE = t.ID_CURSO
		      SET t.TUTOR_ID = bt.TUTOR_ID, 
  				...
  				UPDATE tbl_indicadores_dipp t
        		JOIN carga_base_coord_dipp bt ON bt.GROUP_ID = t.ID_GRUPO_PRESENCIAL AND bt.COURSE = t.ID_CURSO
		      SET t.COORD_ID = bt.COORD_ID, 
	         	...
	         	UPDATE tbl_indicadores_dipp t
					...
					drop table if exists tmp_avaliacao_dipp;
					create temporary table tmp_avaliacao_dipp (
						userid bigint(10),
						course bigint(10),
						assign bigint(10),
						lastsub bigint(20),
					...
					alter table tmp_avaliacao_dipp add index idx_uc (userid, course);


					insert into tmp_avaliacao_dipp
							select 
						...
						join tbl_indicadores_dipp tcc on tcc.USERID=asu.userid and tcc.ID_CURSO=ma.course 
					...
					update tbl_indicadores_dipp t 
					join tmp_avaliacao_dipp b on t.USERID=b.userid and t.ID_CURSO=b.course
      ...
      WHILE U_QUIZ_ < 4 DO
          ...
          SET S_QUIZ_ = 0;
            ...
            WHILE S_QUIZ_ < 4 DO
                ...
                SET @query = CONCAT('UPDATE tbl_indicadores_dipp t 
      ...
      WHILE U_QUIZ_ < 4 DO
       ...
       END WHILE;
            ...
            UPDATE tbl_indicadores_dipp t 
				SET t.QUIZ_ROTEIRO_ESTUDO = IFNULL(
																(SELECT CAST(MAX(gg.FINALGRADE) AS decimal(3, 1))
																 FROM mdl_grade_grades gg
																 JOIN mdl_grade_items gi ON gi.id = gg.ITEMID 
																 WHERE gg.USERID = t.USERID
																       AND gi.courseid = t.ID_CURSO
																        AND gi.itemname REGEXP 'ROTEIRO.*ESTUDO.*DISCIPLINA'),-1
															 );
      	
 		DROP TABLE IF EXISTS carga_base_prof_dipp;
 		DROP TABLE IF EXISTS carga_base_coord_dipp;

update  tbl_indicadores_dipp set atividade_1_aluno='Nunca Acessou' WHERE atividade_1_aluno IS NULL;
...
where a.UltimoAcessoDisciplina='Nunca Acessou' and
	...
	UPDATE tbl_prc_log pl
		SET pl.FIM = sysdate(),
	 		 pl.`STATUS` = CODE_, 
	 		 pl.REGISTROS = IFNULL(REGISTROS_, (SELECT COUNT(1) FROM tbl_indicadores_dipp)),
	 		 pl.INFO = MSG_
	 WHERE pl.ID = ID_LOG_;
