...
-- Copiando estrutura para procedure prod_kls.PRC_ALOCAR_TUTORES_HIBV
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_ALOCAR_TUTORES_HIBV`()
BEGIN


UPDATE anh_import_aluno_curso PARTITION (HIBV) iac
...
DECLARE ALUNOS CURSOR FOR
		...
		(select count(1) from anh_aluno_matricula where username=anh.username and tutor LIKE 'tutor_hib\_%')
		+
		(select count(1) from anh_import_aluno_curso partition (HIBV) where status is null and username=anh.username) as qtd
	FROM anh_import_aluno_curso PARTITION (HIBV) anh
...
SELECT id INTO V_MODELO FROM mdl_sgt_modelo WHERE sigla='HIB';

DROP TABLE IF EXISTS controle_tutores_hib;
CREATE TABLE controle_tutores_hib (
	username VARCHAR(30),
	qtd_alunos INT
);
ALTER TABLE controle_tutores_hib ADD PRIMARY KEY (username);


INSERT INTO controle_tutores_hib 
SELECT TUTOR, count(distinct USERNAME) AS ALUNOS
FROM anh_aluno_matricula anh
JOIN mdl_course c on c.shortname=anh.shortname
WHERE SIGLA='HIBV' AND TUTOR REGEXP 'tutor_hib'
GROUP BY TUTOR
ORDER BY TUTOR;
...
LEFT JOIN (
	...
	FROM anh_aluno_matricula PARTITION (HIBV)
	GROUP BY TUTOR
) a on a.tutor=mdl.username
where a.tutor is null AND U.status='A';


DROP TABLE IF EXISTS controle_tutores_hib_uc;
CREATE TABLE controle_tutores_hib_uc (
	username VARCHAR(30),
	unidade varchar(20),
	curso varchar(20),
	qtd_alunos INT
);
ALTER TABLE controle_tutores_hib_uc ADD PRIMARY KEY (username,unidade,curso);

INSERT INTO controle_tutores_hib_uc 
SELECT * FROM (
SELECT 
	t.username,
	anh.unidade,
...
FROM controle_tutores_hib t
JOIN anh_aluno_matricula anh on anh.tutor=t.username 
GROUP BY t.username,anh.unidade,anh.curso 
) A; 

DROP TABLE IF EXISTS controle_hib_disciplinas;

CREATE TABLE controle_hib_disciplinas AS
	SELECT TUTOR, SHORTNAME
	FROM anh_aluno_matricula PARTITION (HIBV)
	GROUP BY TUTOR, SHORTNAME;
ALTER TABLE `controle_hib_disciplinas`
...
ALUNO: LOOP
			...
			AREALOOP: LOOP
				...
				TUTORDOALUNO: BEGIN 
					...
					IF (V_PM>1) THEN 
						...
						SELECT 
							TUTOR INTO V_TUTOR
						FROM anh_aluno_matricula PARTITION (HIBV) mat 
					...
					IF (V_TUTOR IS NOT NULL) THEN 
							...
							FROM controle_hib_disciplinas WHERE TUTOR=V_TUTOR AND SHORTNAME<>V_SHORTNAME;
				...
				SELECIONATUTOR: BEGIN 
					...
					ORDER BY QTD_ALUNOS_TUTOR DESC;
		...
		IF (V_TUTOR IS NULL) THEN
			NOVOUSUARIO: BEGIN
				...
				IF (T_USUARIOID IS NOT NULL) THEN
					...
					SET V_TUTOR=CONCAT('tutor_hib_',T_USUARIOID);
					...
					INSERT INTO mdl_user (username, password, firstname, lastname, email, city, country, lang, timezone, confirmed, mnethostid, 
								...
								VALUES(V_TUTOR,md5('tutor@kroton'),'TUTOR',' HIB' ,'sucess@simulator.amazones.com','','BR','pt_br_utf8','-3.0',1,1, 
					...
					INSERT INTO controle_tutores_hib (username, qtd_alunos) VALUES (V_TUTOR,0);
	...
	IF (V_TUTOR IS NOT NULL) THEN 	
		...
		SET V_LOG=matricular(V_ANH_ID, V_IDPC, V_IDPT, V_USERNAME, V_SHORTNAME, V_UNIDADE, V_CURSO, V_GRUPO, V_TUTOR, 'student', 'HIBV');
		IF (V_LOG LIKE 'Erro%') THEN 
			SET V_STATUS='ERRO';
		ELSE 
			UPDATE controle_tutores_hib SET qtd_alunos=qtd_alunos+ADDALUNO WHERE username=V_TUTOR;
			SET V_COUNT=0;
			...
			IF V_COUNT=0 THEN 
				INSERT INTO controle_tutores_hib_uc (USERNAME, CURSO, UNIDADE, QTD_ALUNOS)			
				VALUES (V_TUTOR, V_CURSO, V_UNIDADE, 1);				
			ELSE 
				UPDATE controle_tutores_hib_uc SET qtd_alunos=qtd_alunos+ADDALUNO_UC WHERE username=V_TUTOR AND unidade=V_UNIDADE AND curso=V_CURSO;
			END IF;
			
			REPLACE INTO controle_hib_disciplinas (TUTOR, SHORTNAME) 
...
FROM (
	...
	JOIN anh_aluno_matricula PARTITION (HIBV) di ON ed.username=di.username
