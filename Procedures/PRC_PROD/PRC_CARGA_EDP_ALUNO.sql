...
-- Copiando estrutura para procedure prod_kls.PRC_CARGA_EDP_ALUNO
DELIMITER //
CREATE DEFINER=`mveras`@`%` PROCEDURE `PRC_CARGA_EDP_ALUNO`()
BEGIN

DELETE FROM anh_import_aluno_curso PARTITION (EDP) WHERE STATUS IS NULL;



DROP TABLE IF EXISTS alunos_edb_mdl;
CREATE TEMPORARY TABLE alunos_edb_mdl AS SELECT DISTINCT md5(CONCAT(USERNAME, SHORTNAME, GRUPO)) AS CHAVE 
FROM anh_aluno_matricula PARTITION (EDP);
ALTER TABLE alunos_edb_mdl ADD PRIMARY KEY pk_shortname (chave);

DROP TABLE IF EXISTS alunos_edb_kls;
CREATE TEMPORARY TABLE alunos_edb_kls AS 
	...
	WHERE LENGTH(c.cd_agrupamento)>0 AND cd.shortname LIKE 'ED%';
	
ALTER TABLE alunos_edb_kls ADD PRIMARY KEY pk_shortname (chave);	

DELETE t1
FROM alunos_edb_mdl t1
JOIN alunos_edb_kls t2 on t1.chave=t2.chave;
	...
	FROM anh_aluno_matricula PARTITION (EDP) anh
	JOIN anh_import_aluno_curso iac on iac.ID=anh.id_matricula_atual 
	JOIN alunos_edb_mdl b on b.chave=md5(concat(anh.username, anh.shortname, anh.grupo));
...
DELETE t1
FROM alunos_edb_kls t1
JOIN alunos_edb_mdl t2 on t1.chave=t2.chave;
	...
	JOIN alunos_edb_kls anh ON anh.chave=md5(CONCAT(p.login,cd.shortname,t.shortname))
  WHERE 
  		LENGTH(c.cd_agrupamento)>0 and 
  		cd.shortname LIKE 'ED%' and 
   	p.FL_ETL_ATU != 'E' AND 
		pap.ds_papel = 'ALUNO'
	GROUP BY p.login,cd.shortname,i.cd_instituicao,i.nm_ies,c.cd_agrupamento,c.nm_curso,d.cd_disciplina,d.ds_disciplina,t.shortname;
