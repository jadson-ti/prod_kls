...
-- Copiando estrutura para procedure prod_kls.PRC_ALOCAR_TUTORES_DIB
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_ALOCAR_TUTORES_DIB`()
BEGIN


UPDATE anh_import_aluno_curso PARTITION (DIBV) iac
...
DECLARE ALUNOS CURSOR FOR
		...
		(select count(1) from anh_aluno_matricula where username=anh.username and tutor LIKE 'tutor_dib\_%')
		+
		(select count(1) from anh_import_aluno_curso partition (DIBV) where status is null and username=anh.username) as qtd
	FROM anh_import_aluno_curso PARTITION (DIBV) anh
...
SELECT id INTO V_MODELO FROM mdl_sgt_modelo WHERE sigla='DIB';
...
JOIN mdl_course c on c.shortname=anh.shortname
WHERE SIGLA='DIBV' AND TUTOR REGEXP 'tutor_dib'
GROUP BY TUTOR
ORDER BY TUTOR;
...
ALUNO: LOOP
			...
			AREALOOP: LOOP
				...
				TUTORDOALUNO: BEGIN 
					...
					IF (V_PM>1) THEN 
						...
						SELECT 
							TUTOR INTO V_TUTOR
						FROM anh_aluno_matricula PARTITION (DIBV) mat 
		...
		IF (V_TUTOR IS NULL) THEN
			NOVOUSUARIO: BEGIN
				...
				IF (T_USUARIOID IS NOT NULL) THEN
					...
					SET V_TUTOR=CONCAT('tutor_dib_',T_USUARIOID);
					...
					INSERT INTO mdl_user (username, password, firstname, lastname, email, city, country, lang, timezone, confirmed, mnethostid, 
								...
								VALUES(V_TUTOR,md5('tutor@kroton'),'TUTOR',' DIB' ,'sucess@simulator.amazones.com','','BR','pt_br_utf8','-3.0',1,1, 
	...
	IF (V_TUTOR IS NOT NULL) THEN 	
		...
		SET V_LOG=matricular(V_ANH_ID, V_IDPC, V_IDPT, V_USERNAME, V_SHORTNAME, V_UNIDADE, V_CURSO, V_GRUPO, V_TUTOR, 'student', 'DIBV');
...
FROM (
	...
	JOIN anh_aluno_matricula PARTITION (DIBV) di ON ed.username=di.username
