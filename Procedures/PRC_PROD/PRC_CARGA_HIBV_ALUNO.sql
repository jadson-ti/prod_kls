...
-- Copiando estrutura para procedure prod_kls.PRC_CARGA_HIBV_ALUNO
DELIMITER //
CREATE DEFINER=`mveras`@`%` PROCEDURE `PRC_CARGA_HIBV_ALUNO`()
BEGIN


DELETE FROM anh_import_aluno_curso PARTITION (HIBV) WHERE STATUS IS NULL;



DROP TABLE IF EXISTS alunos_hibv_mdl;
CREATE TEMPORARY TABLE alunos_hibv_mdl AS SELECT DISTINCT md5(CONCAT(USERNAME, SHORTNAME)) AS CHAVE 
FROM anh_aluno_matricula PARTITION (HIBV);
ALTER TABLE alunos_hibv_mdl ADD PRIMARY KEY pk_shortname (chave);

DROP TABLE IF EXISTS alunos_hibv_kls;
CREATE TEMPORARY TABLE alunos_hibv_kls AS 
	...
	WHERE LENGTH(c.cd_agrupamento)>0 AND p.FL_ETL_ATU != 'E' AND cd.shortname regexp '^HAM';
	
ALTER TABLE alunos_hibv_kls ADD PRIMARY KEY pk_shortname (chave);	

DELETE t1
FROM alunos_hibv_mdl t1
JOIN alunos_hibv_kls t2 on t1.chave=t2.chave;
	...
	FROM anh_aluno_matricula PARTITION (HIBV) anh
	JOIN anh_import_aluno_curso iac on iac.ID=anh.id_matricula_atual 
	JOIN alunos_hibv_mdl b on b.chave=md5(concat(anh.username,anh.shortname));
		
CALL PRC_CARGA_MATRICULAS;

DELETE anh 
FROM anh_aluno_matricula PARTITION (HIBV) anh
JOIN anh_import_aluno_curso PARTITION (HIBV) iac on anh.id_matricula_atual=iac.id
JOIN alunos_hibv_mdl b on b.chave=md5(concat(anh.username,anh.shortname));
...
DELETE t1
FROM alunos_hibv_kls t1
JOIN alunos_hibv_mdl t2 on t1.chave=t2.chave;
	...
	JOIN alunos_hibv_kls anh ON anh.chave=md5(CONCAT(p.login,cd.shortname))
  WHERE 
  		LENGTH(c.cd_agrupamento)>0 AND 
   	p.FL_ETL_ATU != 'E'  AND cd.shortname regexp '^HAM' AND
		pap.ds_papel = 'ALUNO'
