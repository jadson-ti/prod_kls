...
-- Copiando estrutura para procedure prod_kls.PRC_RETORNO_NT_DI2B
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_RETORNO_NT_DI2B`()
BEGIN
    ...
    DECLARE c_cursos CURSOR FOR
            SELECT 
                    di.SHORTNAME,
                    (
                        SELECT COUNT(gi.id) FROM mdl_course tc
                        ...
                        WHERE (gi.itemname REGEXP '^U1' OR gi.itemname REGEXP '^U2')
                            AND (gi.itemname REGEXP 'dade$' OR gi.itemname REGEXP 'agem$' OR gi.itemname REGEXP 'tica$')
                            AND tc.SHORTNAME = di.SHORTNAME
                    ) AS QTD_ATIVIDADE
            FROM tbl_retorno_nt_di2b di
    ...
    SELECT database(), 'PRC_RETORNO_NT_DI2B', user(), sysdate(), 'PROCESSANDO' FROM DUAL;
    ...
    DROP TABLE IF EXISTS tbl_retorno_nt_di2b;

    CREATE TABLE tbl_retorno_nt_di2b (
     ...
     CREATE INDEX idx_NT2_DI2 ON tbl_retorno_nt_di2b (SHORTNAME, USERNAME);

    INSERT INTO tbl_retorno_nt_di2b (CD_INSTITUICAO, NM_IES, CD_PESSOA, USERNAME, NOME_USUARIO, COURSE_ID, SHORTNAME, 
    ...
    OPEN c_cursos;
        ...
        IF v_finished = 1 THEN LEAVE get_CURSOS; END IF;
        ...
        UPDATE tbl_retorno_nt_di2b di1
        JOIN tbl_retorno_base2 rb ON rb.USERNAME = di1.USERNAME AND rb.SHORTNAME = di1.SHORTNAME
    ...
    UPDATE tbl_prc_log pl
        SET pl.FIM = sysdate(),
              pl.`STATUS` = CODE_, 
              pl.REGISTROS = IFNULL(REGISTROS_, (SELECT COUNT(1) FROM tbl_retorno_nt_di2b)),
              pl.INFO = MSG_
     WHERE pl.ID = ID_LOG_;
