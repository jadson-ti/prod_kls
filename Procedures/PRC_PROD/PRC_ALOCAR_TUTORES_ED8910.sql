...
-- Copiando estrutura para procedure prod_kls.PRC_ALOCAR_TUTORES_ED8910
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_ALOCAR_TUTORES_ED8910`()
BEGIN
...
DECLARE ALUNOS CURSOR FOR
		...
		(select count(1) from anh_aluno_matricula PARTITION (ESTV) where username=anh.username)	AS qtd_est
	FROM anh_import_aluno_curso PARTITION (EDV) anh
	JOIN mdl_user u on u.username=anh.username AND u.mnethostid=1
	JOIN mdl_course c on c.shortname=anh.shortname AND c.summary REGEXP '^ED([89]|10).*'
	WHERE 
		GRUPO='GRUPO_' AND 
		STATUS IS NULL AND 
		LOG IS NULL AND 
...
JOIN anh_aluno_matricula PARTITION (EDV) anh on anh.tutor=t.username 
...
ALUNO: LOOP
	...
	FETCH ALUNOS INTO V_ANH_ID, V_IDPC, V_IDPT, V_USERNAME, V_COURSE_ID, V_SHORTNAME, V_UNIDADE, V_CURSO, V_PM, V_PM_EST;
	...
	ALOCACAO: BEGIN 	
		...
		TUTORESATIVOS: BEGIN 
			...
			AREALOOP: LOOP
				...
				IF (V_TUTOR IS NULL AND V_PM>1) THEN 
						SELECT 
							TUTOR INTO V_TUTOR
