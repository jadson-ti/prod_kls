...
-- Copiando estrutura para procedure prod_kls.PRC_CARGA_DIP_ALUNO
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_CARGA_DIP_ALUNO`()
BEGIN

DELETE FROM anh_import_aluno_curso PARTITION (DIP) WHERE STATUS IS NULL;



DROP TABLE IF EXISTS alunos_dip_mdl;
CREATE TEMPORARY TABLE alunos_dip_mdl AS SELECT DISTINCT md5(CONCAT(USERNAME, SHORTNAME, GRUPO)) AS CHAVE 
FROM anh_aluno_matricula PARTITION (DIP);
ALTER TABLE alunos_dip_mdl ADD PRIMARY KEY pk_shortname (chave);

DROP TABLE IF EXISTS alunos_dip_kls;
CREATE TEMPORARY TABLE alunos_dip_kls AS 
...
ALTER TABLE alunos_dip_kls ADD PRIMARY KEY pk_shortname (chave);	

DELETE t1
FROM alunos_dip_mdl t1
JOIN alunos_dip_kls t2 on t1.chave=t2.chave;
	...
	FROM anh_aluno_matricula PARTITION (DIP) anh
	JOIN anh_import_aluno_curso iac on iac.ID=anh.id_matricula_atual 
	JOIN alunos_dip_mdl b on b.chave=md5(concat(anh.username,anh.shortname,anh.grupo));
		
CALL PRC_CARGA_MATRICULAS;

DELETE anh 
FROM anh_aluno_matricula PARTITION (DIP) anh
JOIN anh_import_aluno_curso PARTITION (DIP) iac on anh.id_matricula_atual=iac.id
JOIN alunos_dip_mdl b on b.chave=md5(concat(anh.username,anh.shortname,anh.grupo));
...
DELETE t1
FROM alunos_dip_kls t1
JOIN alunos_dip_mdl t2 on t1.chave=t2.chave;
	...
	JOIN alunos_dip_kls anh ON anh.chave=md5(CONCAT(p.login,cd.shortname,t.shortname))
  WHERE 
