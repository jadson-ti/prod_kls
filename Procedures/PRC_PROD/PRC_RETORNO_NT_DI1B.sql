...
-- Copiando estrutura para procedure prod_kls.PRC_RETORNO_NT_DI1B
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_RETORNO_NT_DI1B`()
BEGIN
	...
	DECLARE c_cursos CURSOR FOR
             SELECT DISTINCT di1.SHORTNAME,
             					  (SELECT COUNT(gi.itemname)
										  FROM mdl_course c2
										  JOIN mdl_grade_items gi ON gi.courseid = c2.id
																		 AND gi.itemtype = 'mod'
																		 AND gi.itemmodule = 'quiz'
																		 AND gi.itemname REGEXP '^1B'
											JOIN mdl_course_modules cm ON cm.course = c2.id
												AND cm.idnumber <> 'teacher_class'
												AND cm.instance = gi.iteminstance
                                                AND cm.module IN (select id from mdl_modules where name = 'quiz') 
										 WHERE c2.shortname = di1.SHORTNAME) AS AUSENCIA
					FROM tbl_retorno_nt_di1b di1;
   ...
   DECLARE CONTINUE HANDLER FOR NOT FOUND 
	...
	INSERT INTO tbl_prc_log (BANCO, OBJETO, AUTOR, INICIO, `STATUS`)
		SELECT database(), 'PRC_RETORNO_NT_DI1B', user(), sysdate(), 'PROCESSANDO' FROM DUAL;
	...
	DROP TABLE IF EXISTS tbl_retorno_nt_di1b;

	CREATE TABLE tbl_retorno_nt_di1b (
 	...
 	CREATE INDEX idx_NT2_DI1 ON tbl_retorno_nt_di1b (SHORTNAME);

	INSERT INTO tbl_retorno_nt_di1b (CD_INSTITUICAO, NM_IES, CD_PESSOA, USERNAME, NOME_USUARIO, COURSE_ID, SHORTNAME,
        ...
        JOIN mdl_course_categories cc on cc.id=mc.category and cc.name REGEXP '1'
	...
	OPEN c_cursos;
	  ...
	  END IF;
			...
			START TRANSACTION;
					...
					UPDATE tbl_retorno_nt_di1b di1
	...
	UPDATE tbl_prc_log pl
		SET pl.FIM = sysdate(),
	 		 pl.`STATUS` = CODE_, 
	 		 pl.REGISTROS = IFNULL(REGISTROS_, (SELECT COUNT(1) FROM tbl_retorno_nt_di1b)),
	 		 pl.INFO = MSG_
	 WHERE pl.ID = ID_LOG_;
