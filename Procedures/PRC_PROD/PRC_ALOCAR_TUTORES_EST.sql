...
-- Copiando estrutura para procedure prod_kls.PRC_ALOCAR_TUTORES_EST
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_ALOCAR_TUTORES_EST`()
BEGIN


UPDATE anh_import_aluno_curso PARTITION (ESTV) iac
...
DECLARE ALUNOS CURSOR FOR
	...
	FROM anh_import_aluno_curso PARTITION (ESTV) anh
...
SELECT id INTO V_MODELO FROM mdl_sgt_modelo WHERE sigla='EST';

DROP TABLE IF EXISTS controle_tutores_est;
CREATE TABLE controle_tutores_est (
	username VARCHAR(30),
	qtd_alunos INT
);
ALTER TABLE controle_tutores_est ADD PRIMARY KEY (username);

INSERT INTO controle_tutores_est 
SELECT TUTOR, count(distinct USERNAME) AS ALUNOS
FROM anh_aluno_matricula anh
JOIN mdl_course c on c.shortname=anh.shortname
WHERE (
			(SIGLA='ESTV' AND c.SHORTNAME LIKE 'ESTV%') 
			OR 
			c.summary REGEXP '^ED([89]|10).*'
		) AND TUTOR LIKE 'tutor_est%'
GROUP BY TUTOR
ORDER BY TUTOR;
...
left join controle_tutores_est tmp on tmp.username=mdl.username and mdl.mnethostid=1
LEFT JOIN (
	SELECT TUTOR, count(distinct USERNAME) 
	FROM anh_aluno_matricula PARTITION (ESTV)
	GROUP BY TUTOR
	ORDER BY TUTOR
) a on a.tutor=mdl.username
where a.tutor is null  AND U.status='A' and tmp.username is null;	

DROP TABLE IF EXISTS controle_tutores_est_uc;
CREATE TABLE controle_tutores_est_uc (
	username VARCHAR(30),
	unidade varchar(20),
	curso varchar(20),
	qtd_alunos INT
);
ALTER TABLE controle_tutores_est_uc ADD PRIMARY KEY (username,unidade,curso);

INSERT INTO controle_tutores_est_uc 
SELECT * FROM (
SELECT 
	t.username,
	anh.unidade,
...
ALUNO: LOOP
	...
	ALOCACAO: BEGIN 	
			...
			AREALOOP: LOOP
				...
				TUTORDOALUNO: BEGIN 
					...
					FROM (	
						SELECT 
							TUTOR 
						FROM anh_aluno_matricula PARTITION (ESTV) mat 
						...
						WHERE mat.USERNAME=V_USERNAME  AND mat.shortname LIKE 'ESTV%' AND 
		...
		IF (V_TUTOR IS NULL) THEN
			...
			NOVOUSUARIO: BEGIN
				...
				WHERE um.id IS NULL and umo.id_usuario_id is null and uf.id_area_formacao=V_AF_ID
				...
				IF (T_USUARIOID IS NOT NULL) THEN
					...
					INSERT INTO mdl_user (username, password, firstname, lastname, email, city, country, lang, timezone, confirmed, mnethostid, 
								...
								VALUES(CONCAT('tutor_est_',T_USUARIOID),md5('tutor@kroton'),'TUTOR',' EST' ,'sucess@simulator.amazones.com','','BR','pt_br_utf8','-3.0',1,1, 
								'', '',0, unix_timestamp(now()));
					SET NOVO_MDL_USER=LAST_INSERT_ID();
					SET V_TUTOR=CONCAT('tutor_est_',T_USUARIOID);
					
					INSERT INTO mdl_sgt_usuario_moodle (id_usuario_id,mdl_user_id,mdl_username) 
					VALUES (T_USUARIOID,NOVO_MDL_USER,V_TUTOR);	
					
					INSERT INTO controle_tutores_est (username, qtd_alunos) VALUES (V_TUTOR,0);
	...
	IF (V_TUTOR IS NOT NULL) THEN 	
		...
		SET V_LOG=matricular(V_ANH_ID, V_IDPC, V_IDPT, V_USERNAME, V_SHORTNAME, V_UNIDADE, V_CURSO, V_GRUPO, V_TUTOR, 'student', 'ESTV');
		IF (V_LOG LIKE 'Erro%') THEN 
			SET V_STATUS='ERRO';
		ELSE 
			UPDATE controle_tutores_est SET qtd_alunos=qtd_alunos+ADDALUNO WHERE username=V_TUTOR;
			SET V_COUNT=0;
			...
			IF V_COUNT=0 THEN 
				INSERT INTO controle_tutores_est_uc (USERNAME, CURSO, UNIDADE, QTD_ALUNOS)			
				VALUES (V_TUTOR, V_CURSO, V_UNIDADE, 1);				
			ELSE 
				UPDATE controle_tutores_est_uc SET qtd_alunos=qtd_alunos+ADDALUNO_UC 
				WHERE username=V_TUTOR AND unidade=V_UNIDADE AND curso=V_CURSO;
			END IF;
			
			SET V_STATUS='OK';
		END IF;
		UPDATE anh_import_aluno_curso PARTITION (ESTV) SET TUTOR=V_TUTOR, GRUPO=V_GRUPO, STATUS=V_STATUS, LOG=V_LOG WHERE ID=V_ANH_ID;
	ELSE 
		UPDATE anh_import_aluno_curso PARTITION (ESTV) SET LOG='Não foi possível alocar um tutor, aguardando cadastro' WHERE ID=V_ANH_ID;
   END IF;
...
FROM (
	SELECT DISTINCT
		...
		est.TUTOR AS TUTOR_EST,
		g2.id AS GRUPO_ID_DESTINO,
		est.GRUPO AS GRUPO_EST	
	FROM anh_aluno_matricula PARTITION (EDV) ed
	JOIN mdl_user u on u.username=ed.username and u.mnethostid=1
	JOIN mdl_course c on c.shortname=ed.shortname and c.summary REGEXP '^ED([89]|10).*'
	JOIN mdl_groups g on g.courseid=c.id and g.description=ed.grupo
	JOIN anh_aluno_matricula PARTITION (ESTV) est ON ed.username=est.username
	LEFT JOIN mdl_groups g2 on g2.courseid=c.id and g2.description=est.GRUPO
	WHERE ed.TUTOR REGEXP 'tutor_ed_[0-9].*' and est.shortname LIKE 'ESTV%'
...
JOIN (
	...
	JOIN mdl_groups g on g.courseid=c.id and g.description=ed.grupo
	JOIN anh_aluno_matricula PARTITION (ESTV) est ON ed.username=est.username
	JOIN mdl_groups g2 on g2.courseid=c.id and g2.description=est.GRUPO
	WHERE ed.TUTOR REGEXP 'tutor_ed_[0-9].*' AND est.shortname LIKE 'ESTV%'
...
SELECT DISTINCT
	...
	JOIN mdl_course c on c.shortname=ed.shortname and c.summary REGEXP '^ED([89]|10).*'
	JOIN mdl_groups g on g.courseid=c.id and g.description=ed.grupo
	JOIN anh_aluno_matricula PARTITION (ESTV) est ON ed.username=est.username and est.shortname LIKE 'ESTV%'
	JOIN mdl_groups g2 on g2.courseid=c.id and g2.description=est.GRUPO
...
SET anh.GRUPO=tmp.GRUPO_EST, anh.TUTOR=tmp.TUTOR_EST;
