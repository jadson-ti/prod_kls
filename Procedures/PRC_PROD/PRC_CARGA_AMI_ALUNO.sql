...
-- Copiando estrutura para procedure prod_kls.PRC_CARGA_AMI_ALUNO
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_CARGA_AMI_ALUNO`()
BEGIN

DELETE FROM anh_import_aluno_curso PARTITION (AMI) WHERE STATUS IS NULL;



DROP TABLE IF EXISTS alunos_ami_mdl;
CREATE TEMPORARY TABLE alunos_ami_mdl AS SELECT DISTINCT md5(CONCAT(ami.USERNAME, ami.SHORTNAME, ami.GRUPO)) AS CHAVE 
FROM anh_aluno_matricula PARTITION (AMI) ami
JOIN mdl_user u on u.username=ami.username and u.mnethostid=1
JOIN mdl_role_assignments ra on ra.userid=u.id 
JOIN mdl_context co on co.id=ra.contextid
JOIN mdl_course c on c.id=co.instanceid
JOIN mdl_groups g on g.description=ami.GRUPO and g.courseid=c.id
JOIN mdl_groups_members gm on gm.userid=u.id and gm.groupid=g.id;
ALTER TABLE alunos_ami_mdl ADD PRIMARY KEY pk_shortname (chave);

DROP TABLE IF EXISTS alunos_ami_kls;
CREATE TEMPORARY TABLE alunos_ami_kls AS 
	...
	JOIN kls_lms_tipo_modelo tm ON tm.id_tp_modelo = d.id_tp_modelo AND tm.sigla LIKE 'AMI%'
	JOIN mdl_user u ON u.username=p.login AND u.mnethostid=1
	WHERE length(c.cd_agrupamento)>0 
	;
ALTER TABLE alunos_ami_kls ADD PRIMARY KEY pk_shortname (chave);	

DELETE t1
FROM alunos_ami_mdl t1
JOIN alunos_ami_kls t2 on t1.chave=t2.chave;
	...
	FROM anh_aluno_matricula PARTITION (AMI) anh
	JOIN anh_import_aluno_curso iac on iac.ID=anh.id_matricula_atual 
	JOIN alunos_ami_mdl b on b.chave=md5(concat(anh.username, anh.shortname, anh.grupo));
CALL PRC_CARGA_MATRICULAS;

DELETE anh 
FROM anh_aluno_matricula PARTITION (AMI) anh
JOIN anh_import_aluno_curso PARTITION (AMI) iac on anh.id_matricula_atual=iac.id
JOIN alunos_ami_mdl b on b.chave=md5(concat(anh.username, anh.shortname, anh.grupo));
...
DELETE t1
FROM alunos_ami_kls t1
JOIN alunos_ami_mdl t2 on t1.chave=t2.chave;
	...
	JOIN alunos_ami_kls anh ON anh.chave=md5(CONCAT(p.login,cd.shortname,t.shortname))
  WHERE 
