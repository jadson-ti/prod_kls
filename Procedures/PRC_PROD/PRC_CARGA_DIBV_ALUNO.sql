...
-- Copiando estrutura para procedure prod_kls.PRC_CARGA_DIBV_ALUNO
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_CARGA_DIBV_ALUNO`()
BEGIN

DELETE FROM anh_import_aluno_curso PARTITION (DIBV) WHERE STATUS IS NULL;



DROP TABLE IF EXISTS alunos_dibv_mdl;
CREATE TEMPORARY TABLE alunos_dibv_mdl AS SELECT DISTINCT md5(CONCAT(USERNAME, SHORTNAME)) AS CHAVE 
FROM anh_aluno_matricula PARTITION (DIBV);
ALTER TABLE alunos_dibv_mdl ADD PRIMARY KEY pk_shortname (chave);

DROP TABLE IF EXISTS alunos_dibv_kls;
CREATE TEMPORARY TABLE alunos_dibv_kls AS 
	...
	JOIN kls_lms_tipo_modelo tm ON tm.id_tp_modelo = d.id_tp_modelo AND tm.sigla IN ('DIB','DI')
	JOIN mdl_user u ON u.username=p.login AND u.mnethostid=1;
ALTER TABLE alunos_dibv_kls ADD PRIMARY KEY pk_shortname (chave);	

DELETE t1
FROM alunos_dibv_mdl t1
JOIN alunos_dibv_kls t2 on t1.chave=t2.chave;
	...
	FROM anh_aluno_matricula PARTITION (DIBV) anh
	JOIN anh_import_aluno_curso iac on iac.ID=anh.id_matricula_atual 
	JOIN alunos_dibv_mdl b on b.chave=md5(concat(anh.username,anh.shortname));
		
CALL PRC_CARGA_MATRICULAS;

DELETE anh 
FROM anh_aluno_matricula PARTITION (DIBV) anh
JOIN anh_import_aluno_curso PARTITION (DIBV) iac on anh.id_matricula_atual=iac.id
JOIN alunos_dibv_mdl b on b.chave=md5(concat(anh.username,anh.shortname));
...
DELETE t1
FROM alunos_dibv_kls t1
JOIN alunos_dibv_mdl t2 on t1.chave=t2.chave;
	...
	JOIN alunos_dibv_kls anh ON anh.chave=md5(CONCAT(p.login,cd.shortname))
  WHERE 
