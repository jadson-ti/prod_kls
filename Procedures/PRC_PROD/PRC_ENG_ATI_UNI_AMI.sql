...
-- Copiando estrutura para procedure prod_kls.PRC_ENG_ATI_UNI_AMI
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_ENG_ATI_UNI_AMI`()
BEGIN
	...
	INSERT INTO tbl_prc_log (BANCO, OBJETO, AUTOR, INICIO, `STATUS`)
		SELECT database(), 'PRC_ENG_ATI_UNI_AMI', user(), sysdate(), 'PROCESSANDO' FROM DUAL;
	


	DROP TABLE IF EXISTS tbl_eng_ati_uni_consolidado_ami1;
	
	CREATE TABLE tbl_eng_ati_uni_consolidado_ami1 AS (
		SELECT ind.unidade,
		 		 ind.IDNUMBER_GRUPO, 
				SUM(IF((
	...
	CREATE INDEX idx_C1_AMI ON tbl_eng_ati_uni_consolidado_ami1 (IDNUMBER_GRUPO);
	



	DROP TABLE IF EXISTS tbl_eng_ati_uni_total_ami1;
	
	
	CREATE TABLE tbl_eng_ati_uni_total_ami1 AS (
		SELECT ind.unidade,
				 ind.IDNUMBER_GRUPO,
				 SUM(36) as 'Nenhuma',
				 SUM(IF(ind.QUIZ_U1S1_DIAGNOSTICA		>= 0,1,0))	as '1',
	...
	CREATE INDEX idx_T1_AMI ON tbl_eng_ati_uni_total_ami1 (IDNUMBER_GRUPO);
	



	DROP TABLE IF EXISTS tbl_eng_ati_uni_consolidado_ami2;
	
	
	CREATE TABLE tbl_eng_ati_uni_consolidado_ami2 AS (
	...
	CREATE INDEX idx_C2_AMI ON tbl_eng_ati_uni_consolidado_ami2 (IDNUMBER_GRUPO);




	DROP TABLE IF EXISTS tbl_eng_ati_uni_total_ami2;
	
	
	CREATE TABLE tbl_eng_ati_uni_total_ami2 AS (
		SELECT ind.nome_curso_kls,
				 ind.IDNUMBER_GRUPO,
				 SUM(36) as 'Nenhuma',
				 SUM(IF(ind.QUIZ_U1S1_DIAGNOSTICA		>= 0,1,0))	as '1',
	...
	CREATE INDEX idx_T2_AMI ON tbl_eng_ati_uni_total_ami2 (IDNUMBER_GRUPO);
