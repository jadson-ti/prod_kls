...
-- Copiando estrutura para procedure prod_kls.PRC_CARGA_ESTVP_ALUNO
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_CARGA_ESTVP_ALUNO`()
BEGIN

DELETE FROM anh_import_aluno_curso PARTITION (ESTVP) WHERE STATUS IS NULL;



DROP TABLE IF EXISTS alunos_tccp_mdl;
CREATE TEMPORARY TABLE alunos_tccp_mdl AS 
SELECT DISTINCT md5(CONCAT(USERNAME, SHORTNAME, GRUPO)) AS CHAVE 
FROM anh_aluno_matricula PARTITION (ESTVP);
ALTER TABLE alunos_tccp_mdl ADD PRIMARY KEY pk_shortname (chave);

DROP TABLE IF EXISTS alunos_tccp_kls;
CREATE TEMPORARY TABLE alunos_tccp_kls AS 
	...
	where cd.shortname LIKE 'EST%';
ALTER TABLE alunos_tccp_kls ADD PRIMARY KEY pk_shortname (chave);	

DELETE t1
FROM alunos_tccp_mdl t1
JOIN alunos_tccp_kls t2 on t1.chave=t2.chave;
	...
	FROM anh_aluno_matricula PARTITION (ESTVP) anh
	JOIN anh_import_aluno_curso iac on iac.ID=anh.id_matricula_atual 
	JOIN alunos_tccp_mdl b on b.chave=md5(concat(anh.username, anh.shortname, anh.grupo));
...
CREATE TEMPORARY TABLE alunos_tccp_kls AS 
	...
	JOIN kls_lms_tipo_modelo tm ON tm.id_tp_modelo = d.id_tp_modelo AND tm.sigla='EST'
	JOIN mdl_user u ON u.username=p.login AND u.mnethostid=1
	where cd.shortname LIKE 'ESTV%' AND length(c.cd_agrupamento)>0;
ALTER TABLE alunos_tccp_kls ADD PRIMARY KEY pk_shortname (chave);	

DELETE t1
FROM alunos_tccp_kls t1
JOIN alunos_tccp_mdl t2 on t1.chave=t2.chave;
	...
	JOIN alunos_tccp_kls anh ON anh.chave=md5(CONCAT(p.login,cd.shortname,t.shortname))  
  WHERE 
	   length(c.cd_agrupamento)>0 and 
		cd.shortname LIKE 'ESTV%' AND 
   	p.FL_ETL_ATU  REGEXP '[NA]' AND 
		pap.ds_papel = 'ALUNO'
	GROUP BY p.login,cd.shortname,i.cd_instituicao,i.nm_ies,c.cd_curso,c.nm_curso,d.cd_disciplina,d.ds_disciplina;
