...
-- Copiando estrutura para procedure prod_kls.PRC_CARGA_PVDP_ALUNO
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_CARGA_PVDP_ALUNO`()
BEGIN

DELETE FROM anh_import_aluno_curso PARTITION (PVDP) WHERE STATUS IS NULL;



DROP TABLE IF EXISTS alunos_pvdp_mdl;
CREATE TEMPORARY TABLE alunos_pvdp_mdl AS SELECT DISTINCT md5(CONCAT(USERNAME, SHORTNAME, GRUPO)) AS CHAVE 
FROM anh_aluno_matricula PARTITION (PVDP);
ALTER TABLE alunos_pvdp_mdl ADD PRIMARY KEY pk_shortname (chave);

DROP TABLE IF EXISTS alunos_pvdp_kls;
CREATE TEMPORARY TABLE alunos_pvdp_kls AS 
	...
	WHERE LENGTH(c.cd_agrupamento)>0 AND cd.shortname='ED_PDV';
ALTER TABLE alunos_pvdp_kls ADD PRIMARY KEY pk_shortname (chave);	

DELETE t1
FROM alunos_pvdp_mdl t1
JOIN alunos_pvdp_kls t2 on t1.chave=t2.chave;
	...
	FROM anh_aluno_matricula PARTITION (PVDP) anh
	JOIN anh_import_aluno_curso iac on iac.ID=anh.id_matricula_atual 
	JOIN alunos_pvdp_mdl b on b.chave=md5(concat(anh.username,anh.shortname,anh.grupo));
		
CALL PRC_CARGA_MATRICULAS;

DELETE anh 
FROM anh_aluno_matricula PARTITION (PVDP) anh
JOIN anh_import_aluno_curso PARTITION (PVDP) iac on anh.id_matricula_atual=iac.id
JOIN alunos_pvdp_mdl b on b.chave=md5(concat(anh.username,anh.shortname,anh.grupo));
...
DELETE t1
FROM alunos_pvdp_kls t1
JOIN alunos_pvdp_mdl t2 on t1.chave=t2.chave;
	...
	JOIN kls_lms_tipo_modelo tm ON tm.id_tp_modelo = d.id_tp_modelo
	JOIN mdl_user u ON u.username=p.login AND u.mnethostid=1
	JOIN alunos_pvdp_kls anh ON anh.chave=md5(CONCAT(p.login,cd.shortname,t.shortname))
	WHERE 
  		LENGTH(c.cd_agrupamento)>0 AND 
		cd.shortname='ED_PDV' AND
   	p.FL_ETL_ATU != 'E' AND 
		pap.ds_papel = 'ALUNO'
