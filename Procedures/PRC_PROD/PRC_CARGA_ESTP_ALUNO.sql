...
-- Copiando estrutura para procedure prod_kls.PRC_CARGA_ESTP_ALUNO
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_CARGA_ESTP_ALUNO`()
BEGIN

DELETE FROM anh_import_aluno_curso PARTITION (ESTP) WHERE STATUS IS NULL;



DROP TABLE IF EXISTS alunos_estp_mdl;
CREATE TEMPORARY TABLE alunos_estp_mdl AS SELECT DISTINCT md5(CONCAT(USERNAME, SHORTNAME, GRUPO)) AS CHAVE 
FROM anh_aluno_matricula PARTITION (ESTP);
ALTER TABLE alunos_estp_mdl ADD PRIMARY KEY pk_shortname (chave);

DROP TABLE IF EXISTS alunos_estp_kls;
CREATE TEMPORARY TABLE alunos_estp_kls AS 
...
ALTER TABLE alunos_estp_kls ADD PRIMARY KEY pk_shortname (chave);	

DELETE t1
FROM alunos_estp_mdl t1
JOIN alunos_estp_kls t2 on t1.chave=t2.chave;
	...
	FROM anh_aluno_matricula PARTITION (ESTP) anh
	JOIN anh_import_aluno_curso iac on iac.ID=anh.id_matricula_atual 
	JOIN alunos_estp_mdl b on b.chave=md5(concat(anh.username, anh.shortname, anh.grupo));
...
CREATE TEMPORARY TABLE alunos_estp_kls AS 
	...
	JOIN kls_lms_tipo_modelo tm ON tm.id_tp_modelo = d.id_tp_modelo AND tm.sigla='EST'
	JOIN mdl_user u ON u.username=p.login AND u.mnethostid=1
	where length(c.cd_agrupamento)>0 AND p.fl_etl_atu<>'E';
ALTER TABLE alunos_estp_kls ADD PRIMARY KEY pk_shortname (chave);	

DELETE t1
FROM alunos_estp_kls t1
JOIN alunos_estp_mdl t2 on t1.chave=t2.chave;
	...
	JOIN alunos_estp_kls anh ON anh.chave=md5(CONCAT(p.login,cd.shortname,t.shortname))
  WHERE 
