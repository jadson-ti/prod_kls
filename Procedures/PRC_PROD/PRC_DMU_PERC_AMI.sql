...
-- Copiando estrutura para procedure prod_kls.PRC_DMU_PERC_AMI
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_DMU_PERC_AMI`()
BEGIN
	...
	INSERT INTO tbl_prc_log (BANCO, OBJETO, AUTOR, INICIO, `STATUS`)
		SELECT database(), 'PRC_DMU_PERC_AMI', user(), sysdate(), 'PROCESSANDO' FROM DUAL;
	


	DROP TABLE IF EXISTS tbl_dmu_perc_consolidado_ami1;
	
	CREATE TABLE tbl_dmu_perc_consolidado_ami1 AS (
		SELECT ind.UNIDADE,
		 		 ind.IDNUMBER_GRUPO, 
	...
	CREATE INDEX idx_C1_AMI ON tbl_dmu_perc_consolidado_ami1 (IDNUMBER_GRUPO);
	



	DROP TABLE IF EXISTS tbl_dmu_perc_total_ami1;
	
	
	CREATE TABLE tbl_dmu_perc_total_ami1 AS (
		SELECT ind.UNIDADE,
				 ind.IDNUMBER_GRUPO,
	...
	CREATE INDEX idx_T1_AMI ON tbl_dmu_perc_total_ami1 (IDNUMBER_GRUPO);
	



	DROP TABLE IF EXISTS tbl_dmu_perc_consolidado_ami2;
	
	
	CREATE TABLE tbl_dmu_perc_consolidado_ami2 AS (
	...
	CREATE INDEX idx_C2_AMI ON tbl_dmu_perc_consolidado_ami2 (IDNUMBER_GRUPO);




	DROP TABLE IF EXISTS tbl_dmu_perc_total_ami2;
	
	
	CREATE TABLE tbl_dmu_perc_total_ami2 AS (
		SELECT ind.NOME_CURSO_KLS AS curso,
				 ind.IDNUMBER_GRUPO,
				 (IF(QUIZ_U1S1_DIAGNOSTICA > 0, 1	,0) +
				 IF(QUIZ_U1S1_APRENDIZAGEM > 0, 1	,0) +
	...
	CREATE INDEX idx_T2_AMI ON tbl_dmu_perc_total_ami2 (IDNUMBER_GRUPO);
