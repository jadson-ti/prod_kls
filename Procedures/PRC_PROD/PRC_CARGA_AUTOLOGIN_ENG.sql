...
-- Copiando estrutura para procedure prod_kls.PRC_CARGA_AUTOLOGIN_ENG
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_CARGA_AUTOLOGIN_ENG`()
BEGIN
...
CREATE TEMPORARY TABLE alunos_kls_mdl AS 
	SELECT DISTINCT md5(CONCAT(USERNAME, 'ENG')) AS CHAVE 
...
CREATE TEMPORARY TABLE alunos_kls_kls AS 
	...
	SELECT DISTINCT md5(CONCAT(p.login, 'ENG')) AS CHAVE 
	FROM kls_lms_pessoa p
	JOIN kls_lms_pessoa_curso pc ON pc.ID_PESSOA=p.id_pessoa AND pc.termo_turma='1' AND pc.fl_etl_atu REGEXP '[N|A]'
...
JOIN alunos_kls_kls t2 on t1.chave=t2.chave;
		...
		JOIN alunos_kls_mdl mat on mat.chave=md5(CONCAT(anh.username,'ENG'))
		WHERE anh.SIGLA='ENG';
...
JOIN alunos_kls_mdl b on b.chave=md5(concat(anh.username, 'ENG'))
WHERE anh.SIGLA='ENG';
...
JOIN alunos_kls_mdl t2 on t1.chave=t2.chave;
	   ...
	   INSERT INTO anh_import_aluno_curso (ID_PESSOA_CURSO,ID_PES_CUR_DISC, ID_PESSOA_TURMA,SIGLA,ORIGEM,USERNAME,SHORTNAME,CODIGO_POLO,NOME_POLO,
		...
		'AUTOLOGINENG' AS CODIGO_DISCIPLINA,
		'AUTOLOGINENG' AS NOME_DISCIPLINA, 
		'1' AS SERIE, 
		c.turno AS TURNO,
		NULL AS TURMA,
		'ENGAUTOLOGIN' AS GRUPO,
		'student' AS ROLE, 
		date_format(now(),'%Y-%m-%d') AS DATA_IMPORTACAO,
	   'I' AS SITUACAO,
		NULL AS STATUS,
	...
	JOIN kls_lms_papel pap ON pap.id_papel = pc.id_papel 
	JOIN mdl_user u ON u.username=p.login AND u.mnethostid=1
	JOIN alunos_kls_kls anh ON anh.chave=md5(CONCAT(p.login,'ENG'))
  WHERE 
  		p.FL_ETL_ATU != 'E' AND 
		pap.ds_papel = 'ALUNO';
