...
-- Copiando estrutura para procedure prod_kls.PRC_LOG_USR_CUR_CMID_MDL
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_LOG_USR_CUR_CMID_MDL`()
BEGIN
	...
	OPEN c_cursos;
	  ...
	  END IF;
		...
		SELECT IFNULL(MAX(t.id), 0) INTO nowid_ 
		  FROM tbl_log_usr_cur_cmid_mdl t 
		...
		WHILE nowid_ <  maxid_ DO
			...
			START TRANSACTION;
				...
				INSERT INTO tbl_log_usr_cur_cmid_mdl
					SELECT t.id,
							 t.userid,
							 t.courseid,
							 t.component,
