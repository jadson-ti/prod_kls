...
-- Copiando estrutura para procedure prod_kls.PRC_CARGA_AMP_ALUNO
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_CARGA_AMP_ALUNO`()
BEGIN

DELETE FROM anh_import_aluno_curso PARTITION (AMP) WHERE STATUS IS NULL;



DROP TABLE IF EXISTS alunos_amp_mdl;
CREATE TEMPORARY TABLE alunos_amp_mdl AS SELECT DISTINCT md5(CONCAT(USERNAME, SHORTNAME, GRUPO)) AS CHAVE 
FROM anh_aluno_matricula PARTITION (AMP);
ALTER TABLE alunos_amp_mdl ADD INDEX pk_shortname (chave);

DROP TABLE IF EXISTS alunos_amp_kls;
CREATE TEMPORARY TABLE alunos_amp_kls AS 
	...
	JOIN kls_lms_tipo_modelo tm ON tm.id_tp_modelo = d.id_tp_modelo AND tm.sigla LIKE 'AMP%'
	JOIN mdl_user u ON u.username=p.login AND u.mnethostid=1
	WHERE p.fl_etl_atu REGEXP '[NA]';
ALTER TABLE alunos_amp_kls ADD PRIMARY KEY pk_shortname (chave);	

DELETE t1
FROM alunos_amp_mdl t1
JOIN alunos_amp_kls t2 on t1.chave=t2.chave;
	...
	FROM anh_aluno_matricula PARTITION (AMP) anh
	JOIN anh_import_aluno_curso iac on iac.ID=anh.id_matricula_atual 
	JOIN alunos_amp_mdl b on b.chave=md5(concat(anh.username, anh.shortname, anh.grupo));

CALL PRC_CARGA_MATRICULAS;

DELETE anh 
FROM anh_aluno_matricula PARTITION (AMP) anh
JOIN anh_import_aluno_curso PARTITION (AMP) iac on anh.ID_MATRICULA_ATUAL=iac.id
JOIN alunos_amp_mdl b on b.chave=md5(concat(anh.username, anh.shortname, anh.grupo));
...
CREATE TEMPORARY TABLE alunos_amp_kls AS 
	...
	JOIN kls_lms_tipo_modelo tm ON tm.id_tp_modelo = d.id_tp_modelo AND tm.sigla LIKE 'AMP%'
	JOIN mdl_user u ON u.username=p.login AND u.mnethostid=1
	WHERE LENGTH(c.cd_agrupamento)>0 AND p.fl_etl_atu REGEXP '[NA]';
ALTER TABLE alunos_amp_kls ADD PRIMARY KEY pk_shortname (chave);	

DELETE t1
FROM alunos_amp_kls t1
JOIN alunos_amp_mdl t2 on t1.chave=t2.chave;
			...
			JOIN alunos_amp_kls anh ON anh.chave=md5(CONCAT(p.login,cd.shortname,t.shortname))
		  WHERE 
