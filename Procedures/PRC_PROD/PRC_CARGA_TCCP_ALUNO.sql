...
-- Copiando estrutura para procedure prod_kls.PRC_CARGA_TCCP_ALUNO
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_CARGA_TCCP_ALUNO`()
BEGIN

DELETE FROM anh_import_aluno_curso PARTITION (TCCP) WHERE STATUS IS NULL;
...
FROM anh_aluno_matricula PARTITION (TCCP);
...
JOIN alunos_tccp_kls t2 on t1.chave=t2.chave;
	...
	FROM anh_aluno_matricula PARTITION (TCCP) anh
...
DROP TABLE IF EXISTS alunos_tccp_mdl;
CREATE TEMPORARY TABLE alunos_tccp_mdl AS SELECT DISTINCT md5(CONCAT(USERNAME, SHORTNAME, GRUPO)) AS CHAVE 
FROM anh_aluno_matricula PARTITION (TCCP);
ALTER TABLE alunos_tccp_mdl ADD PRIMARY KEY pk_shortname (chave);
