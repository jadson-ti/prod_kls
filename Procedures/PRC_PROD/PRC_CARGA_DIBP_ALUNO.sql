...
-- Copiando estrutura para procedure prod_kls.PRC_CARGA_DIBP_ALUNO
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_CARGA_DIBP_ALUNO`()
BEGIN

DELETE FROM anh_import_aluno_curso PARTITION (DIBP) WHERE STATUS IS NULL;
...
FROM anh_aluno_matricula PARTITION (DIBP);
...
DELETE t1
FROM alunos_dib_mdl t1
JOIN alunos_dib_kls t2 on t1.chave=t2.chave;
	...
	FROM anh_aluno_matricula PARTITION (DIBP) anh
	JOIN anh_import_aluno_curso iac on iac.ID=anh.id_matricula_atual 
	JOIN alunos_dib_mdl b on b.chave=md5(concat(anh.username,anh.shortname,anh.grupo));
		
CALL PRC_CARGA_MATRICULAS;

DELETE anh 
FROM anh_aluno_matricula PARTITION (DIBP) anh
JOIN anh_import_aluno_curso PARTITION (DIBP) iac on anh.id_matricula_atual=iac.id
JOIN alunos_dib_mdl b on b.chave=md5(concat(anh.username,anh.shortname,anh.grupo));
