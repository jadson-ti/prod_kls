...
-- Copiando estrutura para procedure prod_kls.PRC_CARGA_HIBAMPP_ALUNO
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_CARGA_HIBAMPP_ALUNO`()
BEGIN

DELETE FROM anh_import_aluno_curso PARTITION (HAMPP) WHERE STATUS IS NULL;



DROP TABLE IF EXISTS alunos_hibamp_mdl;
CREATE TEMPORARY TABLE alunos_hibamp_mdl AS SELECT DISTINCT md5(CONCAT(USERNAME, SHORTNAME, GRUPO)) AS CHAVE
FROM anh_aluno_matricula PARTITION (HAMPP);
ALTER TABLE alunos_hibamp_mdl ADD PRIMARY KEY pk_shortname (chave);

DROP TABLE IF EXISTS alunos_hibamp_kls;
CREATE TEMPORARY TABLE alunos_hibamp_kls AS
...
where p.FL_ETL_ATU != 'E' and cd.shortname regexp '^HAMP' ;
ALTER TABLE alunos_hibamp_kls ADD PRIMARY KEY pk_shortname (chave);    



DELETE t1
FROM alunos_hibamp_mdl t1
JOIN alunos_hibamp_kls t2 on t1.chave=t2.chave;
    ...
    FROM anh_aluno_matricula PARTITION (HAMPP) anh
    JOIN anh_import_aluno_curso iac on iac.ID=anh.id_matricula_atual
    JOIN alunos_hibamp_mdl b on b.chave=md5(concat(anh.username,anh.shortname,anh.grupo));
...
DELETE anh
FROM anh_aluno_matricula PARTITION (HAMPP) anh
JOIN anh_import_aluno_curso PARTITION (HAMPP) iac on anh.id_matricula_atual=iac.id
JOIN alunos_hibamp_mdl b on b.chave=md5(concat(anh.username,anh.shortname,anh.grupo));
...
DELETE t1
FROM alunos_hibamp_kls t1
JOIN alunos_hibamp_mdl t2 on t1.chave=t2.chave;
    ...
    JOIN alunos_hibamp_kls anh ON anh.chave=md5(CONCAT(p.login,cd.shortname,t.shortname))
  WHERE
