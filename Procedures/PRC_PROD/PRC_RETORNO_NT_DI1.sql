...
-- Copiando estrutura para procedure prod_kls.PRC_RETORNO_NT_DI1
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_RETORNO_NT_DI1`()
BEGIN
   ...
   DECLARE CONTINUE HANDLER
	...
	INSERT INTO tbl_prc_log (BANCO, OBJETO, AUTOR, INICIO, `STATUS`)
		SELECT database(), 'PRC_RETORNO_NT_DI1', user(), sysdate(), 'PROCESSANDO' FROM DUAL;
	...
	DROP TABLE IF EXISTS tbl_retorno_nt_di1;

	CREATE TABLE tbl_retorno_nt_di1 (
 	...
 	CREATE INDEX idx_NT2_DI1 ON tbl_retorno_nt_di1 (SHORTNAME);



	START TRANSACTION;

		INSERT INTO tbl_retorno_nt_di1 (CD_INSTITUICAO, NM_IES, CD_PESSOA, USERNAME, NOME_USUARIO, COURSE_ID, SHORTNAME,
   ...
   START TRANSACTION;
	...
	UPDATE tbl_prc_log pl
		SET pl.FIM = sysdate(),
	 		 pl.`STATUS` = CODE_, 
	 		 pl.REGISTROS = IFNULL(REGISTROS_, (SELECT COUNT(1) FROM tbl_retorno_nt_di1)),
	 		 pl.INFO = MSG_
	 WHERE pl.ID = ID_LOG_;
