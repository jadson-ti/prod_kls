...
-- Copiando estrutura para procedure prod_kls.PRC_ALOCAR_TUTORES_ED1234
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_ALOCAR_TUTORES_ED1234`()
BEGIN
...
DECLARE ALUNOS CURSOR FOR
		...
		(select count(1) from anh_aluno_matricula where username=anh.username and tutor LIKE 'TUTOR\_ED\_%')
		+
	...
	JOIN mdl_course c on c.shortname=anh.shortname AND c.summary REGEXP '^ED[1234]$'
	WHERE 
		GRUPO='GRUPO_' AND 
		STATUS IS NULL AND 
		LOG IS NULL AND 
			...
			AREALOOP: LOOP
				...
				TUTORDOALUNO: BEGIN 
					...
					IF (V_PM>1) THEN 
						...
						WHERE mat.USERNAME=V_USERNAME AND uf.id_area_formacao IN (
				...
				SELECIONATUTOR: BEGIN 
					...
					DECLARE TUTORES CURSOR FOR
						...
						JOIN (
							...
							FROM mdl_sgt_usuario u
							JOIN mdl_sgt_usuario_formacao uf on uf.id_usuario=u.id and uf.id_area_formacao IN (
										SELECT id_area_formacao 
										FROM mdl_sgt_formacao_disciplina 
										WHERE shortname=V_SHORTNAME
							)
