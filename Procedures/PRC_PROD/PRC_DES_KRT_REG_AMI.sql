...
-- Copiando estrutura para procedure prod_kls.PRC_DES_KRT_REG_AMI
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_DES_KRT_REG_AMI`()
BEGIN
	...
	INSERT INTO tbl_prc_log (BANCO, OBJETO, AUTOR, INICIO, `STATUS`)
		SELECT database(), 'PRC_DES_KRT_REG_AMI', user(), sysdate(), 'PROCESSANDO' FROM DUAL;
	


	DROP TABLE IF EXISTS tbl_des_krt_reg_consolidado_ami1;
	
	CREATE TABLE tbl_des_krt_reg_consolidado_ami1 AS (
		SELECT sum((IF(QUIZ_U1S1_DIAGNOSTICA >= 6, 1	,0) +
				 ...
				 IF(QUIZ_U4						>= 6,	1	,0)))  AS soma_notas,
				 ...
				 IF(QUIZ_U4						> 0,	1	,0))) AS total_notas_validas
	...
	DROP TABLE IF EXISTS tbl_des_krt_reg_total_ami1;
	
	
	CREATE TABLE tbl_des_krt_reg_total_ami1 AS (
		SELECT sum((IF(QUIZ_U1S1_DIAGNOSTICA > 0, 1	,0) +
				 IF(QUIZ_U1S1_APRENDIZAGEM > 0, 1	,0) +
				 IF(QUIZ_U1S2_DIAGNOSTICA	> 0, 1	,0) +
				 IF(QUIZ_U1S2_APRENDIZAGEM > 0, 1	,0) +
				 IF(QUIZ_U1S3_DIAGNOSTICA	> 0, 1	,0) +
	...
	DROP TABLE IF EXISTS tbl_des_krt_reg_consolidado_ami2;
	
	
	CREATE TABLE tbl_des_krt_reg_consolidado_ami2 AS (
	...
	DROP TABLE IF EXISTS tbl_des_krt_reg_total_ami2;
	
	
	CREATE TABLE tbl_des_krt_reg_total_ami2 AS (
		SELECT rg.nm_regional,
				 				 sum((IF(QUIZ_U1S1_DIAGNOSTICA > 0, 1	,0) +
				 IF(QUIZ_U1S1_APRENDIZAGEM > 0, 1	,0) +
				 IF(QUIZ_U1S2_DIAGNOSTICA	> 0, 1	,0) +
