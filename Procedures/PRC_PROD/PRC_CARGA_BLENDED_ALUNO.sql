...
-- Copiando estrutura para procedure prod_kls.PRC_CARGA_BLENDED_ALUNO
DELIMITER //
CREATE DEFINER=`fsantini`@`%` PROCEDURE `PRC_CARGA_BLENDED_ALUNO`()
BEGIN

DELETE FROM anh_import_aluno_curso PARTITION (DIB) WHERE STATUS IS NULL;
	...
	FROM anh_aluno_matricula PARTITION (DIB) anh
	...
	JOIN kls_lms_pessoa_turma pt on pt.id_pes_turma_disciplina=anh.id_pessoa_turma 
	WHERE 
   	(pcd.FL_ETL_ATU='E' OR pt.FL_ETL_ATU='E' OR cd.FL_ETL_ATU='E');
		
CALL PRC_CARGA_MATRICULAS;

DELETE anh 
FROM anh_aluno_matricula anh
JOIN anh_import_aluno_curso PARTITION (DIB) iac on 
iac.USERNAME=anh.username and iac.shortname=anh.shortname 
and anh.ID_PES_CUR_DISC=iac.ID_PES_CUR_DISC
and anh.ID_PESSOA_TURMA=iac.ID_PESSOA_TURMA
WHERE iac.DATA_IMPORTACAO=DATE_FORMAT(NOW(),'%Y-%m-%d') AND SITUACAO='B' AND STATUS='OK';



DROP TABLE IF EXISTS alunos_dib_mdl;
CREATE TEMPORARY TABLE alunos_dib_mdl AS SELECT DISTINCT md5(CONCAT(USERNAME, SHORTNAME, GRUPO)) AS CHAVE 
FROM anh_aluno_matricula PARTITION (DIB);
ALTER TABLE alunos_dib_mdl ADD PRIMARY KEY pk_shortname (chave);

DROP TABLE IF EXISTS alunos_dib_kls;
CREATE TEMPORARY TABLE alunos_dib_kls AS 
	...
	JOIN kls_lms_tipo_modelo tm ON tm.id_tp_modelo = d.id_tp_modelo AND tm.sigla='DIB'
	JOIN mdl_user u ON u.username=p.login AND u.mnethostid=1;
ALTER TABLE alunos_dib_kls ADD PRIMARY KEY pk_shortname (chave);	

DELETE t1
FROM alunos_dib_kls t1
JOIN alunos_dib_mdl t2 on t1.chave=t2.chave;
	...
	JOIN alunos_dib_kls anh ON anh.chave=md5(CONCAT(p.login,cd.shortname,t.shortname))
  WHERE 
   	p.FL_ETL_ATU != 'E' AND 
		pap.ds_papel = 'ALUNO'
